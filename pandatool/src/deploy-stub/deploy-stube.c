...
static void *map_blob(off_t offset, size_t size, const char *path) {
    ...
    fseek(runtime, blobinfo.blob_offset, SEEK_SET);
...
/**
 * Loads the header for the binary
 */
static uint64_t get_uint64(char * bytes, int pos){
    uint64_t val;
    memcpy( &val, bytes+pos, sizeof( uint64_t ) );
    return val;
}

static uint16_t get_uint16(char * bytes, int pos){
    uint16_t val;
    memcpy( &val, bytes+pos, sizeof( uint16_t ) );
    return val;
}
...
static void *map_header(const char *path) {
    ...
    fread(buffer, filelen, 1, runtime);
    fclose(runtime);
    blobinfo.blob_offset = get_uint64(buffer, 0);
    blobinfo.blob_size = get_uint64(buffer, 8);
    blobinfo.version = get_uint16(buffer, 16);
    blobinfo.num_pointers = get_uint16(buffer, 18);
    blobinfo.codepage = get_uint16(buffer, 20);
    blobinfo.flags = get_uint16(buffer, 22);
    blobinfo.reserved = get_uint64(buffer, 24);
    int pos = 24+8;
    for (int i = 0; i < blobinfo.num_pointers; i++) {
        blobinfo.pointers[i] = get_uint64(buffer, pos);
        pos += 8;
    }
    return NULL;
}
...
int main(int argc, char *argv[]) {
  ...
  if (blobinfo.version == 0 || blobinfo.blob_offset != 0) {
    void *blob = map_blob((off_t)blobinfo.blob_offset, (size_t)blobinfo.blob_size, get_blob_path(argv[0]));
    assert(blob != NULL);
