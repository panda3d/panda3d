...
/**
 * Returns the surface associated with the given polygon, or NULL if no surface
 * is associated.
 */
...
/**
 * Returns true if there is a UV of the indicated name associated with the given
 * vertex of the indicated polygon, false otherwise.  If true, fills in uv with
 * the value.  This performs a lookup in the optional "discontinuous" vertex
 * mapping, which provides the ability to map different UV's per each polygon
 * for the same vertex.  If the UV is not defined here, it may also be defined
 * in the standard vertex map, which is associated with the points themselves.
 */
bool CLwoPolygons::
