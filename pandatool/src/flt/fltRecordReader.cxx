...
/**
 * Returns true if some error has been encountered while reading (for instance,
 * a truncated file).
 */
bool FltRecordReader::
error() const {
 ...
 * opcode and record length.  We need read the next header in advance so we can
 * check to see if it happens to be a continuation record.  If it is, we will
 * need to concatenate the records together before returning.
 */
