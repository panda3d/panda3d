...
get_transform_step(int n) const {
  nassertr(n >= 0 && n < (int)_transform_steps.size(),
           (const FltTransformRecord *)nullptr);
  return _transform_steps[n];
}
...
extract_ancillary(FltRecordReader &reader) {
  FltTransformRecord *step = (FltTransformRecord *)nullptr;
  ...
  // A transform step.
  nassertr(step != (FltTransformRecord *)nullptr, false);
