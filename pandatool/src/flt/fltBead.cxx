...
/**
 * Returns the single-precision 4x4 matrix that represents the transform applied
 * to this bead, or the identity matrix if the bead has not been transformed.
 */
...
/**
 * Returns the number of individual steps that define the net transform on this
 * bead as returned by set_transform().  Each step is a single transformation;
 * the concatenation of all transformations will produce the matrix represented
 * by set_transform().
 */
int FltBead::
...
/**
 * Applies the indicated transform step to the net transformation applied to the
 * bead.
 */
void FltBead::
...
/**
 * Returns the replicate count of this bead.  If this is nonzero, it means that
 * the bead is implicitly copied this number of additional times (for
 * replicate_count + 1 total copies), applying the transform on this bead for
 * each copy.  In this case, the transform does *not* apply to the initial copy
 * of the bead.
 */
int FltBead::
 ...
 * count to some nonzero number, you must also set a transform on the bead.  See
 * set_replicate_count().
 */
void FltBead::
...
/**
 * Reads a transform matrix ancillary bead.  This defines the net transformation
 * that has been applied to the bead, and precedes the set of individual
 * transform steps that define how this net transform was computed.
 */
bool FltBead::
