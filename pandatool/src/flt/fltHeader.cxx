...
FltHeader(PathReplace *path_replace) : FltBeadID(this) {
  if (path_replace == (PathReplace *)nullptr) {
...
get_instance(int instance_index) const {
  ...
  return (FltInstanceDefinition *)nullptr;
}
...
get_light_source(int light_index) const {
  ...
  return (FltLightSourceDefinition *)nullptr;
}
...
get_eyepoint(int n) {
  nassertr(n >= 0 && n < get_num_eyepoints(), (FltEyepoint *)nullptr);
  return &_eyepoints[n];
}
...
get_trackplane(int n) {
  nassertr(n >= 0 && n < get_num_trackplanes(), (FltTrackplane *)nullptr);
