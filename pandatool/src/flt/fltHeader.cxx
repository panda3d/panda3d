...
/**
 * The FltHeader constructor accepts a PathReplace pointer; it uses this object
 * to automatically convert all external filename and texture references.  (This
 * is necessary because the FltHeader has to look in the same directory as the
 * texture to find the .attr file, so it must pre-convert at least the texture
 * references.)  Most of the other file converters do not have this requirement,
 * so they do not need to pre-convert any pathname references.
 */
FltHeader::
...
/**
 * Replaces the PathReplace object (which specifies how to mangle paths from the
 * source to the destination file) with a new one.
 */
void FltHeader::
 ...
 * If the converter is non-const, this returns a non-const pointer, which can be
 * adjusted.
 */
...
/**
 * Attempts to read a complete Flt file from the already-opened stream.  Returns
 * FE_ok on success, otherwise on failure.
 */
FltError FltHeader::
read_flt(istream &in) {
...
/**
 * Attempts to write a complete Flt file to the already-opened stream.  Returns
 * FE_ok on success, otherwise on failure.
 */
FltError FltHeader::
 ...
 * write_flt() is called.  There are three possibilities:  AU_none: the .attr
 * files are not written automatically; they must be written explicitly via a
 * call to FltTexture::write_attr_data() if you want them to be written.
 * AU_if_missing: the .attr files are written only if they do not already exist.
 * This will not update any .attr files, even if the data is changed.
 * AU_always: the .attr files are always rewritten, even if they already exist
 * and even if the data has not changed.  The default is AU_if_missing.
 */
void FltHeader::
...
/**
 * Returns the latest flt version number that this codebase is known to support
 * (times 100).  Later versions might work, but then again they may not.
 */
int FltHeader::
max_flt_version() {
  return 1570;
 ...
 * version number, and prints a warning to the user if this is not so--the read
 * may or may not succeed.  Returns true if the version number is acceptable
 * (and no warning is printed), or false if it is questionable (and a warning is
 * printed).
 */
bool FltHeader::
check_version() const {
...
/**
 * Returns the units indicated by the flt header, or DU_invalid if the units in
 * the header are not understood.
 */
 ...
 * hierarchy; it marks geometry that may be instanced to various beads elsewhere
 * in the hierarchy by creating a corresponding FltInstanceRef bead.
 */
void FltHeader::
 ...
 * vertex pointer.  If there is no such vertex in the palette, this generates an
 ...
 * includes all different colors, and represents the complete range of alloable
 * color indices.  This is different from the actual number of color entries as
 * read directly from the color palette, since each color entry defines a number
 * of different intensity levels--the value returned by get_num_colors() is
 * equal to get_num_color_entries() * get_num_color_shades().
 */
int FltHeader::
...
/**
 * Returns the color index of the nearest color in the palette that matches the
 * given four-component color, including alpha.
 */
int FltHeader::
...
/**
 * Returns the color index of the nearest color in the palette that matches the
 * given three-component color, ignoring alpha.
 */
int FltHeader::
...
/**
 * Returns the number of actual entries in the color palette.  This is based on
 * the version of the flt file, and is usually either 512 or 1024.
 */
int FltHeader::
...
/**
 * Decodes a MultiGen color, as stored on a face or vertex, into an actual four-
 * component LColor.  Normally you need not call this directly; there are color
 * accessors defined on faces and vertices that do this.
 */
LColor FltHeader::
...
/**
 * Returns the material associated with the given index, or NULL if there is no
 * such material.
 */
 ...
 * the material's index number.  If there is already a material defined for that
...
/**
 * Defines a new texture.  The texture is added in the position indicated by the
 * texture's index number.  If there is already a texture defined for that index
...
/**
 * Returns the light source associated with the given index, or NULL if there is
...
/**
 * Sets the state of the eyepoint/trackplane palette flag.  When this is false,
 * the palette is believed to be meaningless, and will not be written; when it
 * is true, the palette is believed to contain at least some meaningful data,
 * and will be written.
 */
void FltHeader::
...
/**
 * Returns the number of eyepoints in the eyepoint/trackplane palette.  This is
 * presently fixed at 10, according to the MultiGen specs.
 */
int FltHeader::
 ...
 * was read in.  The return value is the total length of the vertex palette,
...
/**
 * Writes whatever ancillary records are required for this bead.  Returns FE_ok
 * on success, or something else on error.
 */
FltError FltHeader::
