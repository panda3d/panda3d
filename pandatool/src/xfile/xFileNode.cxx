...
/**
 * Returns the index number of the child with the indicated name, if any, or -1
 * if none.
 */
int XFileNode::
...
/**
 * If has_guid() returned true, returns the particular GUID associated with this
 * node.
 */
 ...
 * instance of the template.  If the file strictly uses standard templates, the
 * presence of template definitions is optional.  If this returns true, the node
 * must be of type XFileTemplate.
 */
bool XFileNode::
 ...
 * with the indicated name, or false otherwise.  This returns also returns true
 * for references to standard objects.  If this returns true, the node must be
 * of type XFileDataNode (it is either an XFileDataNodeTemplate or an
...
/**
 * This is called on the template that defines an object, once the data for the
 * object has been parsed.  It is responsible for identifying which component of
 * the template owns each data element, and packing the data elements
 * appropriately back into the object.  It returns true on success, or false on
 * an error (e.g.  not enough data elements, mismatched data type).
 */
bool XFileNode::
...
/**
 * Transforms the indicated egg name to a name that is acceptable for a node in
 * the X File format.
 */
string XFileNode::
