...
/**
 * Changes the focus of the View.  By default, the View reports the entire time
 * for the frame, and all of the Collectors that are directly parented to
 ...
 * its immediate parents.  When you constrain the view, you may also specify
 * whether the view should show time data or level data for the indicated
 * collector.  If level data, it reports the levels for the collector, and all
 * of its children; otherwise, it collects the elapsed time.  Changing the
 * constraint causes the current frame's data to become invalidated; you must
 * then call set_to_frame() again to get any useful data out.
 */
void PStatView::
...
/**
 * Supplies the View with the data for the current frame.  This causes the View
 * to update all of its internal data to reflect the frame's data, subject to
 * the current constraint.  It is possible that calling this will increase the
 * total number of reported levels (for instance, if this frame introduced a new
 * collector that hadn't been active previously).  In this case, the caller must
 * update its display or whatever to account for the new level.
 */
void PStatView::
...
/**
 * Returns the total value accounted for by the frame (or by whatever Collector
 * we are constrained to).  This is the sum of all of the individual levels'
 * get_net_value() value.
 */
double PStatView::
get_net_value() const {
...
/**
 * Returns true if there is a level defined for the particular collector, false
 * otherwise.
 */
bool PStatView::
 ...
 * If there is no such level in the view, one will be created--use with caution.
 * Check has_level() first if you don't want this behavior.
 */
 ...
 * the client might change its mind from time to time about who the Collector is
 * parented to, we have to update this dynamically.  Returns true if any change
 * was made to the level's hierarchy, false otherwise.
 */
bool PStatView::
