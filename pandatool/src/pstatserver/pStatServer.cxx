 ...
 * connections.  This may be called more than once to listen simulataneously on
 * multiple connections, as if that were at all useful.  The default parameter,
 * -1, indicates the use of whatever port number has been indicated in the
 * Config file.  This function returns true if the port was successfully opened,
 * or false if it could not open the port.
 */
bool PStatServer::
listen(int port) {
  if (port < 0) {
 ...
 * redefined to return true on this class and also on all PStatMonitors in use.
...
/**
 * An alternative to repeatedly calling poll(), this function yields control of
 * the program to the PStatServer.  It does not return until the program is
 * done.  If interrupt_flag is non-NULL, it is the address of a bool variable
 * that is initially false, and may be asynchronously set true to indicate the
...
/**
 * This should be redefined to return true in derived classes that want to deal
 * with multithreaded readers and such.  If this returns true, the manager will
 * create the listener in its own thread, and thus the PStatReader constructors
 * at least will run in a different thread.  This is not related to the question
 * of whether the reader can handle multiple different PStatThreadDatas; it's
 * strictly a question of whether the readers themselves can run in a separate
 * thread.
 */
bool PStatServer::
is_thread_safe() {
