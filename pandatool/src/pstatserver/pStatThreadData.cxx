...
has_frame(int frame_number) const {
  ...
  return (rel_frame >= 0 && rel_frame < (int)_frames.size() &&
          _frames[rel_frame] != (PStatFrameData *)nullptr);
}
...
get_frame(int frame_number) const {
  ...
  while (rel_frame >= 0 && _frames[rel_frame] == (PStatFrameData *)nullptr) {
    rel_frame--;
  }
  if (rel_frame < 0) {
  ...
  if (rel_frame >= 0 && rel_frame < num_frames) {
    ...
    nassertr(frame != (PStatFrameData *)nullptr, _null_frame);
...
get_frame_number_at_time(double time, int hint) const {
  ...
  while (i >= 0) {
    ...
    if (frame != (PStatFrameData *)nullptr && frame->get_start() <= time) {
      break;
    }
    --i;
  }
...
record_new_frame(int frame_number, PStatFrameData *frame_data) {
  nassertv(frame_data != (PStatFrameData *)nullptr);
  ...
  while (!_frames.empty() &&
         (_frames.front() == (PStatFrameData *)nullptr ||
          _frames.front()->is_empty() ||
          _frames.front()->get_start() < oldest_allowable_time)) {
    if (_frames.front() != (PStatFrameData *)nullptr) {
  ...
  if (_frames[index] != (PStatFrameData *)nullptr) {
...
compute_elapsed_frames() {
  if (_frames.empty()) {
    ...
    while (_now_i > 0 && _frames[_now_i] == (PStatFrameData *)nullptr) {
      _now_i--;
    }
    if (_now_i < 0) {
      ...
      nassertv(_frames[_now_i] != (PStatFrameData *)nullptr);
      ...
      while (old_i >= 0) {
        ...
        if (frame != (PStatFrameData *)nullptr) {
      ...
      nassertv(_then_i >= 0);
      nassertv(_frames[_then_i] != (PStatFrameData *)nullptr);
