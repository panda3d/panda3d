...
/**
 * Returns true if we have received data for the indicated frame number from the
 * client and we still have it stored, or false otherwise.
 */
bool PStatThreadData::
...
/**
 * Returns a FrameData structure associated with the indicated frame number.  If
 * the frame data has not yet been received from the client, returns the newest
 ...
 * time.  If the hint is nonnegative, it represents a frame number that we
 * believe the correct answer to be near, which may speed the search for the
 * frame.
 */
int PStatThreadData::
...
/**
 * Computes the oldest frame number not older than pstats_average_time seconds,
 * and the newest frame number.  Handy for computing average frame rate over a
 * time.  Returns true if there is any data in that range, false otherwise.
 */
bool PStatThreadData::
...
/**
 * Computes the average frame rate over the past pstats_average_time seconds, by
 * counting up the number of frames elapsed in that time interval.
 */
double PStatThreadData::
...
/**
 * Makes room for and stores a new frame's worth of data.  Calling this function
 * may cause old frame data to be discarded to make room, according to the
 * amount of time set up via set_history().  The pointer will become owned by
 * the PStatThreadData object and will be freed on destruction.
 */
void PStatThreadData::
