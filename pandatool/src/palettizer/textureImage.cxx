...
TextureImage::
TextureImage() {
  _preferred_source = (SourceTextureImage *)nullptr;
...
get_preferred_source() {
  if (_preferred_source != (SourceTextureImage *)nullptr) {
  ...
  SourceTextureImage *best = (SourceTextureImage *)nullptr;
  int best_size = 0;
  ...
  for (si = _sources.begin(); si != _sources.end(); ++si) {
    ...
    if (source->get_egg_count() > 0 || !any_referenced) {
      ...
      if (source->exists() && source->get_size()) {
        ...
        if (best == (SourceTextureImage *)nullptr) {
          best = source;
  ...
  if (best == (SourceTextureImage *)nullptr && !_sources.empty()) {
    ...
    if (any_referenced) {
      for (si = _sources.begin();
           si != _sources.end() && best == (SourceTextureImage *)nullptr;
           ++si) {
        SourceTextureImage *source = (*si).second;
        if (source->get_egg_count() > 0) {
          best = source;
...
copy_unplaced(bool redo_all) {
  ...
  for (pi = _placement.begin(); pi != _placement.end(); ++pi) {
    ...
    if (placement->get_omit_reason() != OR_none &&
      ...
      placement->set_dest((DestTextureImage *)nullptr);
    }
  }
...
write_scale_info(ostream &out, int indent_level) {
  ...
  for (pi = _placement.begin(); pi != _placement.end(); ++pi) {
    ...
    if (placement->get_omit_reason() == OR_none) {
      ...
      nassertv(image != (DestTextureImage *)nullptr);
      indent(out, indent_level + 2)
        << "copied to "
        << FilenameUnifier::make_user_filename(image->get_filename());
