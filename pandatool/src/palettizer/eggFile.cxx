...
EggFile::
EggFile() {
  _data = (EggData *)nullptr;
  _first_txa_match = false;
  _default_group = (PaletteGroup *)nullptr;
  _is_surprise = true;
  _is_stale = true;
  _had_data = false;
}
...
void EggFile::
scan_textures() {
  nassertv(_data != (EggData *)nullptr);
...
choose_placements() {
  ...
  for (ti = _textures.begin(); ti != _textures.end(); ++ti) {
    ...
    if (reference->get_placement() != (TexturePlacement *)nullptr &&
      ...
      if (!groups.empty()) {
        ...
        nassertv(placement != (TexturePlacement *)nullptr);
...
bool EggFile::
read_egg(bool noabs) {
  nassertr(_data == (EggData *)nullptr, false);
...
write_description(ostream &out, int indent_level) const {
  ...
  if (_explicitly_assigned_groups.empty()) {
    if (_default_group != (PaletteGroup *)nullptr) {
