...
convert_face(const FltFace *flt_face, FltToEggLevelState &state) {
  ...
  const FltVertexList *vlist = (FltVertexList *)nullptr;
  int num_children = flt_face->get_num_children();
  for (int i = 0; i < num_children && vlist == (FltVertexList *)nullptr; i++) {
  ...
  if (vlist != (FltVertexList *)nullptr) {
...
setup_geometry(const FltGeometry *flt_geom, FltToEggLevelState &state,
  ...
  if (state._flt_object != (FltObject *)nullptr) {
