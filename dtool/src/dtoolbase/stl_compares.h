...
/**
 * This hash_compare class hashes a class object.  It assumes the Key provides a
 * method called get_hash() that returns a size_t.
 */
template<class Key, class Compare = less<Key> >
 ...
 * An STL function object class, this is intended to be used on any ordered
 * collection of pointers to classes that contain an operator ==() method.  It
 * defines the equality of the pointers via operator ==().  Since it doesn't
 * define the ordering of the pointers, it can only be used with hash
 * containers.
 */
template<class Key>
