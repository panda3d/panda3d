...
/**
 * Registers a new type on-the-fly, presumably at runtime.  A new TypeHandle is
 * returned if the typename was not seen before; otherwise the same TypeHandle
...
/**
 * Looks for a previously-registered type with the given id number (as returned
 * by TypeHandle::get_index()). Returns its TypeHandle if it exists, or
...
/**
 * Returns the name of the indicated type.  The "object" pointer is an optional
 * pointer to the TypedObject class that owns this TypeHandle.  It is only used
 * in case the TypeHandle is inadvertantly undefined.
 */
string TypeRegistry::
 ...
 * otherwise.  The "child_object" pointer is an optional pointer to the
 * TypedObject class that owns the child TypeHandle.  It is only used in case
 * the TypeHandle is inadvertently undefined.  This function definition follows
 * the definitions for look_up() and freshen_derivations() just to maximize the
 * chance the the compiler will be able to inline the above functions.  Yeah, a
 * compiler shouldn't care, but there's a big different between "shouldn't" and
 * "doesn't".
 */
bool TypeRegistry::
...
/**
 * Returns the number of root classes--that is, classes that do not inherit from
 * any other classes--known in the system.
 */
int TypeRegistry::
 ...
 * will be 0 if this class does not inherit from any other classes, 1 if normal,
 * single inheritance is in effect, or greater than one if multiple inheritance
 * is in effect.  The "object" pointer is an optional pointer to the TypedObject
 * class that owns this TypeHandle.  It is only used in case the TypeHandle is
...
/**
 * Returns the nth parent class of this type.  The index should be in the range
 * 0 <= index < get_num_parent_classes().
 */
TypeHandle TypeRegistry::
 ...
 * This may then be used to index into get_child_class().  The "object" pointer
 * is an optional pointer to the TypedObject class that owns this TypeHandle.
 * It is only used in case the TypeHandle is inadvertantly undefined.
 */
int TypeRegistry::
...
/**
 * Returns the nth child class of this type.  The index should be in the range 0
 * <= index < get_num_child_classes().
 */
TypeHandle TypeRegistry::
 ...
 * inheritance to the indicated ancestor class.  This is useful in the presence
 * of multiple inheritance to try to determine what properties an unknown type
 * may have.  The "object" pointer is an optional pointer to the TypedObject
 * class that owns this TypeHandle.  It is only used in case the TypeHandle is
 * inadvertantly undefined.
 */
TypeHandle TypeRegistry::
 ...
 * initialize the type anyway.  Assumes the lock is already held.
 */
