...
/**
 * Don't try to construct a PandaSystem object; there is only one of these, and
 * it constructs itself.  Use get_global_ptr() to get a pointer to the one
 * PandaSystem.
 */
PandaSystem::
PandaSystem() :
 ...
 * build of Panda.  When the currently-executing version of Panda was loaded
 * from a distributable package, such as via the browser plugin, then this
 * string will be nonempty and will contain the corresponding version string.
 * You can build applications that use this particular version of Panda by
 * requesting it in the pdef file, using "panda3d", this version string, and the
 * download host provided by get_package_host_url().  If this string is empty,
 * then the currently-executing Panda was built independently, and is not part
 * of a distributable package.  This string is set explicitly at compilation
 * time.  Normally, it should be set to a nonempty string only when building a
 * Panda3D package for distribution.
 */
string PandaSystem::
 ...
 * Panda among distributable Panda versions.  See get_package_version_string()
 * for more information.  This string is set explicitly at compilation time.
 * Normally, it should be set to a nonempty string only when building a Panda3D
 ...
 * dot-delimited integers.  There are usually four integers in this version, but
 * this is not guaranteed.  The Core API is used during the runtime (plugin)
 * environment only.  This may be the empty string if the current version of
 * Panda is not built to provide a particular Core API, which will be the normal
 * case in a development SDK.  However, you should not use this method to
 * determine whether you are running in a runtime environment or not.
 */
string PandaSystem::
 ...
 * the second number of the dotted triple returned by get_version_string().  It
 * changes with each release that introduces new features.
 */
int PandaSystem::
get_minor_version() {
...
/**
 * Returns the sequence version number of the current version of Panda.  This is
 * the third number of the dotted triple returned by get_version_string().  It
 * changes with bugfix updates and very minor feature updates.
 */
int PandaSystem::
...
/**
 * Returns true if current version of Panda claims to be an "official" version,
 * that is, one that was compiled by an official distributor of Panda using a
 * specific version of the panda source tree.  If this is true, there will not
 * be a "c" at the end of the version string returned by get_version_string().
...
/**
 * Returns the number of Panda subsystems that have registered themselves.  This
 * can be used with get_system() to iterate through the entire list of available
 * Panda subsystems.
 */
size_t PandaSystem::
 ...
 * capabilities.  The set of tags and values are per-system and implementation-
 * defined.  The return value is the empty string if the indicated system is
 * undefined or if does not define the indicated tag.
 */
string PandaSystem::
 ...
 * called by ConfigPageManager to preload the value from the panda-package-host-
 * url config variable, for developer's convenience.  This has no effect if the
 * PANDA_PACKAGE_HOST_URL configure variable is defined at compilation time.
 * This also has no effect in NDEBUG mode.
 */
void PandaSystem::
