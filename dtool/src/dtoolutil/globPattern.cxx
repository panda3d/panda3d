 ...
 * Posix glob() function.  Any part of the filename may contain glob characters,
 * including intermediate directory names.  If cwd is specified, it is the
 * directory that relative filenames are taken to be relative to; otherwise, the
 * actual current working directory is assumed.  The return value is the number
 * of files matched, which are added to the results vector.
 */
int GlobPattern::
...
/**
 * The recursive implementation of matches().  This returns true if the pattern
 * substring [pi, pend) matches the candidate substring [ci, cend), false
 * otherwise.
 */
bool GlobPattern::
...
/**
 * Called when an unescaped open square bracked is scanned, this is called with
 * pi positioned after the opening square bracket, scans the set sequence,
 * leaving pi positioned on the closing square bracket, and returns true if the
 * indicated character matches the set of characters indicated, false otherwise.
 */
bool GlobPattern::
