...
#ifdef _WIN32
/**
 * Connects the file stream to the existing OS-defined stream, presumably opened
 * via a low-level OS call.  The filename is for reporting only.  When the file
 * stream is closed, it will also close the underlying OS handle.  This function
 * is the Windows-specific variant.
 */
void PandaFileStreamBuf::
...
/**
 * Reads raw data from the file directly into the indicated buffer.  Returns the
...
/**
 * Writes the indicated buffer directly to the file stream.  Returns the number
 ...
 * characters placed in dest.  This may also set (or read) the value of
 * _last_read_nl, which is preserved from call-to-call to deal with newline
 * combinations that straddle a read operation.
 */
size_t PandaFileStreamBuf::
...
/**
 * Windows case: Converts a buffer from \n to \n\r.  To allow for full buffer
 * expansion, dest_length should be at least 2*source_length.  Returns the
 * number of characters placed in dest.
 */
size_t PandaFileStreamBuf::
 ...
 * conversion at all; but we do strip out \r characters if they appear in the
 * buffer; this will help programmers to realize when they have incorrectly
 * tagged a binary file with text mode, even on a Posix environment.  Returns
 * the number of characters placed in dest.
 */
size_t PandaFileStreamBuf::
...
/**
 * Classic Mac case: Converts a buffer from \n to \r.  Returns the number of
 * characters placed in dest.
 */
size_t PandaFileStreamBuf::
