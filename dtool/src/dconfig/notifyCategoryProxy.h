...
class NotifyCategoryProxy {
public:
  // This should be set to be called at static init time; it
  // initializes the pointer if it is not already.
  NotifyCategory *init();

  // You don't normally need to call these directly, but they're here
  // anyway.  get_unsafe_ptr() assumes the pointer has been
  // initialized; it should be called only when you know static init
  // has completed (i.e. in any function that is not executing at
  // static init time).  get_safe_ptr() should be called when it is
  // possible that static init has not yet completed (i.e. in a
  // function that might execute at static init time); it calls init()
  // first.
  INLINE NotifyCategory *get_unsafe_ptr();
  INLINE NotifyCategory *get_safe_ptr();

  // The following functions, which may be accessed using the
  // proxy.function() syntax, call get_unsafe_ptr().  They should be
  ...
  // The same functions as above, when accessed using
  // proxy->function() syntax, call get_safe_ptr().  These can be used
  // safely either in static-init or non-static-init functions.
...
// Following the config convention, this macro defines an external
// reference to a suitable NotifyCategoryProxy object; it should
...
// NotifyCategoryProxy object of the parent category
