...
/**
 * Returns true if the type has even been typedef'ed and therefore has a simple
 * name available to stand for it.  Extension types are all implicitly
 ...
 * This will typically be a typedef name if one is available, or the full C++
 * name if it is not.  The typedef may or may not be visible within the current
 * scope, so this type name may not be compilable.
 */
string CPPTypeProxy::
