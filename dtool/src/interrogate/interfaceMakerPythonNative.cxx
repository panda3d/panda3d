...
write_class_details(ostream &out, Object *obj) {
  ...
  if (py_subclassable) {
    ...
    std::string ctor_name = obj->_constructors[0]->_ifunc.get_scoped_name();
    out << "/**\n";
    ...
    out << "public:\n";
    out << "  using " << ctor_name << ";\n";
    if (struct_type->is_copy_constructible()) {
      out << "  DtoolProxy_" << ClassName << "(const " << cClassName << " &copy) : " << cClassName << "(copy) { }\n";
    }
    if (struct_type->is_move_constructible()) {
      out << "  DtoolProxy_" << ClassName << "(" << cClassName << " &&from) : " << cClassName << "(std::move(from)) { }\n";
    }
    out << "\n";
    ...
    for (rfi = slots.begin(); rfi != slots.end(); rfi++) {
      ...
      case WT_traverse:
        ...
        // is a low-level function.  Overloads are not supported.
        {
          ...
          out << "  if (local_this->get_ref_count() != ((Dtool_PyInstDef *)self)->_memory_rules) {\n";
          out << "    return 0;\n";
          out << "  }\n\n";
