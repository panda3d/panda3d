 ...
 * whatever lines the remapper needs to the function to set up its return value,
 * e.g.  to declare a temporary variable or something.  It should return the
 * modified expression.
 */
...
/**
 * Returns true if the return value represents a value that was newly allocated,
 * and hence must be explicitly deallocated later by the caller.
 */
bool ParameterRemap::
...
/**
 * If return_value_needs_management() returns true, this should return the index
 * of the function that should be called when it is time to destruct the return
 * value.  It will generally be the same as the destructor for the class we just
 ...
 * particularly in conjunction with the 'new' operator.  If this parameter type
 * is one that will probably give VC++ a headache, this should be set true to
 * indicate that the code generator should save the return value expression into
 * a temporary variable first, and pass the temporary variable name in instead.
 */
bool ParameterRemap::
 ...
 * string, because there's no such type in C (and hence no corresponding CPPType
 * *).
 */
bool ParameterRemap::
