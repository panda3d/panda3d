 ...
 * only one ConfigVariableManager, and it constructs itself.
 */
...
/**
 * The ConfigVariableManager destructor should never be called, because this is
 * a global object that is never freed.
 */
ConfigVariableManager::
~ConfigVariableManager() {
  prc_cat->error()
 ...
 * name; or if a variable with this name has already been created, returns that
 * one instead.
 */
...
/**
 * Defines a variable "template" to match against dynamically-defined variables
 * that may or may not be created in the future.  The template consists of a
 * glob pattern, e.g.  "notify-level-*", which will be tested against any config
 * variable passed to a future call to make_variable().  If the pattern matches,
 * the returned ConfigVariableCore is copied to define the new variable, instead
 * of creating a default, empty one.  This is useful to pre-specify default
 * values for a family of variables that all have similar properties, and all
 * may not be created at the same time.  It is especially useful to avoid
 * cluttering up the list of available variables with user-declared variables
 * that have not been defined yet by the application (e.g.  "egg-object-
 * type-*").  This method basically pre-defines all variables that match the
 ...
 * the prc file; it does not list shadowed values, and it does not list locally-
 * set values.  This is mainly intended for generating a hash of the input
 * config file state.
 */
void ConfigVariableManager::
...
/**
 * Writes a list of all the variables that have been declared somewhere in code,
 ...
 * somewhere in code, along with a brief description.  This is a (usually large)
 * list of config variables that are declared with a generated variable name.
 */
void ConfigVariableManager::
