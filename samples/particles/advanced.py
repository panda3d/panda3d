...
uniform float delta_t;        // current rendering time
...
void main() {
    vec4  col = texture(image, gl_PointCoord) * gl_Color;
...
class Particle:
    ...
    def __buffTex(self, values):
        ...
        # the conversion method depends on Python's version
        convMth = "tostring" if sys.version_info[0] < 3 else "tobytes"
        tex.setRamImage(getattr(data, convMth)())
        return tex
    ...
    def __setShader(self):
        # this method sets the particle shader
        shader = Shader.make(Shader.SL_GLSL, vert, frag)
        self.__nodepath.setShader(shader)
        # the followings are required for setting sprite particles
        shaderAttrib = ShaderAttrib.make(shader)
        shaderAttrib = shaderAttrib.setFlag(ShaderAttrib.F_shader_point_size,
                                            True)
        self.__nodepath.setAttrib(shaderAttrib)
        shader_inputs = [
        ...
        for inp in shader_inputs:
            self.__nodepath.setShaderInput(*inp)

    def __update(self, task):
        pos = self.__emitter.getPos(render)
        self.__nodepath.setShaderInput("emitter_pos", pos)
        self.__nodepath.setShaderInput("delta_t", globalClock.getDt())
        return task.again
