...
class World(DirectObject):
  ...
  def __init__(self):
    ...
    self.robot1.punchLeft = Sequence(
      ...
      Func(self.checkPunch, 2), #Function to check if the punch was successful
  ...
  def checkPunch(self, robot):
    ...
    #punch is directed to robot 1
      ...
      if self.robot1.resetHead.isPlaying(): return 
      ...
      if (not self.robot1.punchLeft.isPlaying() and 
        ...
        if random() > .85: self.robot1.resetHead.start() 
      #Otherwise, only 5% chance of sucessful hit
      elif random() > .95: self.robot1.resetHead.start() 
    else:
      #punch is directed to robot 2, same as above
      if self.robot2.resetHead.isPlaying(): return
      if (not self.robot2.punchLeft.isPlaying() and
        not self.robot2.punchRight.isPlaying()):
        if random() > .85: self.robot2.resetHead.start()
      elif random() > .95: self.robot2.resetHead.start()
