...
# Date: 2019-06-21
#
# This module contains a class to procedurally create label widgets.
...
class Label(Widget):
    ...
    def __create_geometry(cls):
        ...
        # This class generates the shape of a label by first creating 2
        # standard primitives: a sliced torus and a sliced cylinder. Then,
        # copies of these are made for the different corners, resp. sides,
        # and all of their geometry is merged into a single GeomPrimitive.
        ...
        # create the lower right hand corner as a sliced torus
        segments = {"ring": 10, "section": 10, "ring_slice_start_cap": 0,
                    "ring_slice_end_cap": 0, "section_slice_start_cap": 0,
                    "section_slice_end_cap": 0}
        model_maker = TorusMaker(None, .5, .5, segments, ring_slice=270.,
                                 section_slice=180., rotation=180., has_uvs=False)
        corner_lr = model_maker.generate()
        vertex_data = corner_lr.modify_geom(0).modify_vertex_data()
        # the border geometry (corner torus and edge cylinder, below) is
        # created without UV's, but the central rectangle of the label will
        ...
        segments = {"circular": 10, "bottom_cap": 0, "top_cap": 0,
                    "slice_caps_radial": 0}
        model_maker = CylinderMaker((1., -.5, 0.), (0., -.5, 0.), .5, segments,
                                    rotation=90., slice=180., has_uvs=False)
        ...
        # in order to stretch the label
        cls._corner_indices = {"lr": s1, "ur": s2, "ul": s3, "ll": s4}

        # Merge the geometry of all corner torii into one single node
        ...
        for (u, v) in ((1., 0.), (1., 1.), (0., 1.), (0., 0.)):
            values.extend(struct.pack("3f", 0., 0., 1.))
            values.extend(struct.pack("3f", 0., 0., 1.))
            values.extend(struct.pack("2f", u, v))
        ...
        # apply one final rotation to the vertices so the label faces the
        # positive Y-axis
    ...
    def __init__(self, parent, border, text="", text_alignment="center",
        ...
        # by increasing that value for each of the label sides
        borders = (border + 5, border + 5, border + 2, border + 2)

        Widget.__init__(self, "label", parent, borders, stretch_dir=stretch_dir)
