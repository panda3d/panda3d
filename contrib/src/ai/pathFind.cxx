...
/**
 * This function checks for the source and target in the navigation mesh for its
 * availability and then finds the best path via the A* algorithm Then it calls
 * the path follower to make the object follow the path.
 */
...
/**
 * This function is the function which sends the path information one by one to
 * the path follower so that it can store the path needed to be traversed by the
 * pathfinding object
 */
