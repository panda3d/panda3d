 ...
 * processes.  If set_throw_buttons_active is false (which is the default), the
 ...
 * downstream to the child node(s) in the data graph.  A button that *is* on the
 * list will be processed by the ButtonThrower and not passed on to the child
 * node(s).  The return value is true if the button is added, or false if it was
 * already in the set.
 */
bool ButtonThrower::
 ...
 * explicitly processes.  See add_throw_button().  The return value is true if
 * the button is removed, or false if it was not on the set.
 */
bool ButtonThrower::
...
/**
 * Generates an event of the indicated name, adding on all of the user-requested
 * parameters.
 */
void ButtonThrower::
