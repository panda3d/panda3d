...
/**
 * Changes the scale factor applied to forward and backward motion.  The larger
 * this number, the faster the model will move in response to dollying in and
 * out.
 */
void Trackball::
...
/**
 * Returns the invert flag.  When this is set, the inverse matrix is generated,
 * suitable for joining to a camera, instead of parenting the scene under it.
 */
bool Trackball::
get_invert() const {
 ...
 * button serves its normal function.  When it is CM_truck, CM_pan, CM_dolly, or
 * CM_roll, all of the mouse buttons serve the indicated function instead of
 * their normal function.  This can be used in conjunction with some external
 * way of changing modes.
 */
void Trackball::
 ...
 * motion camera relative.  The default is the empty path, which means trackball
...
/**
 * Returns the NodePath that all trackball manipulations are relative to, or the
 * empty path.
 */
