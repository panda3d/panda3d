 ...
 * polygons in the normal, filled mode, or wireframe mode, or in some other yet-
 * to-be-defined mode.  The thickness parameter specifies the thickness to be
 * used for wireframe lines, as well as for ordinary linestrip lines; it also
 * specifies the diameter of points.  (Thick lines are presently only supported
 * in OpenGL; but thick points are supported on either platform.)  If
 * perspective is true, the point thickness represented is actually a width in
 * 3-d units, and the points should scale according to perspective.  When it is
 * false, the point thickness is actually a width in pixels, and points are a
 * uniform screen size regardless of distance from the camera.  In
 * M_filled_wireframe mode, you should also specify the wireframe_color,
 ...
 * other one.  This should return 0 if the two RenderModeAttrib objects are
 ...
 * called with two RenderModeAttrib objects whose get_type() functions return
 * the same.
 */
 ...
 * RenderModeAttrib is encountered in the Bam file.  It should create the
