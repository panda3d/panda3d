...
make_identity() {
  ...
  if (_identity_state == (TransformState *)nullptr) {
...
CPT(TransformState) TransformState::
make_invalid() {
  if (_invalid_state == (TransformState *)nullptr) {
    TransformState *state = new TransformState;
...
validate_composition_cache() const {
  ...
  for (size_t i = 0; i < size; ++i) {
    ...
    if (source != (TransformState *)nullptr) {
...
get_num_unused_states() {
  ...
  for (size_t si = 0; si < size; ++si) {
    ...
    for (i = 0; i < cache_size; ++i) {
      ...
      if (result != (const TransformState *)nullptr && result != state) {
...
int TransformState::
garbage_collect() {
  if (_states == (States *)nullptr || !garbage_collect_states) {
    return 0;
  }
...
return_new(TransformState *state) {
  nassertr(state != (TransformState *)nullptr, state);
  ...
  if (compose_componentwise &&
    ...
    if (paranoid_compose) {
      ...
      if (is_singular()) {
        ...
        nassertr(_inv_mat != (LMatrix4 *)nullptr, make_invalid());
        LMatrix4 new_mat;
...
r_detect_cycles(const TransformState *start_state,
  ...
  for (i = 0; i < cache_size; ++i) {
    ...
    if (result != (const TransformState *)nullptr) {
      if (r_detect_cycles(start_state, result, length + 1,
                          this_seq, cycle_desc)) {
        // Cycle detected.
        if (cycle_desc != (CompositionCycleDesc *)nullptr) {
  ...
  while (!_composition_cache.is_empty()) {
    ...
    if (other != this) {
      ...
      if (oi != -1) {
        ...
        if (ocomp._result != (const TransformState *)nullptr && ocomp._result != other) {
    ...
    // above.)
    if (comp._result != (const TransformState *)nullptr && comp._result != this) {
...
calc_singular() {
  ...
  nassertv(_inv_mat == (LMatrix4 *)nullptr);
