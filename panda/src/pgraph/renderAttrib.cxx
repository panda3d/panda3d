 ...
 * RenderAttrib on a lower node that has a higher override value, or true if the
 * lower RenderAttrib will completely replace the state.  The default behavior
 * is false: normally, a RenderAttrib in the graph cannot completely override a
 * RenderAttrib above it, regardless of its override value--instead, the two
 * attribs are composed.  But for some kinds of RenderAttribs, it is useful to
 * allow this kind of override.  This method only handles the one special case
 * of a lower RenderAttrib with a higher override value.  If the higher
 * RenderAttrib has a higher override value, it always completely overrides.
 * And if both RenderAttribs have the same override value, they are always
 * composed.
 */
bool RenderAttrib::
 ...
 * supposedly-const RenderAttrib objects).
 */
bool RenderAttrib::
validate_attribs() {
  LightReMutexHolder holder(*_attribs_lock);
 ...
 * RenderAttrib pointer for all equivalent RenderAttrib objects.  This is
 * different from return_unique() in that it does not actually guarantee a
 * unique pointer, unless uniquify-attribs is set.
 */
CPT(RenderAttrib) RenderAttrib::
...
/**
 * This function is used by derived RenderAttrib types to share a common
 * RenderAttrib pointer for all equivalent RenderAttrib objects.  The make()
 * function of the derived type should create a new RenderAttrib and pass it
 * through return_new(), which will either save the pointer and return it
 * unchanged (if this is the first similar such object) or delete it and return
 * an equivalent pointer (if there was already a similar object saved).
 */
CPT(RenderAttrib) RenderAttrib::
 ...
 * This should return 0 if the two RenderAttrib objects are equivalent, a number
 ...
 * RenderAttrib objects whose get_type() functions return the same.
 */
int RenderAttrib::
 ...
 * RenderAttrib table.  You must already be holding _attribs_lock before you
 * call this method.
 */
void RenderAttrib::
release_new() {
 ...
 * creates a RenderAttrib object at static init time; it also seems to cause
