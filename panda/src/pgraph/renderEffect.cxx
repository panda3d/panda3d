...
/**
 * Should be overridden by derived classes to return true if adjust_transform()
 * has been defined, and therefore the RenderEffect has some effect on the
 ...
 * true.  Both parameters are in/out.  The original transforms will be passed
 * in, and they may (or may not) be modified in-place by the RenderEffect.
 */
void RenderEffect::
 ...
 * supposedly-const RenderEffect objects).
 */
bool RenderEffect::
validate_effects() {
  if (_effects->empty()) {
 ...
 * RenderEffect pointer for all equivalent RenderEffect objects.  The make()
 * function of the derived type should create a new RenderEffect and pass it
 ...
 * This should return 0 if the two RenderEffect objects are equivalent, a number
 ...
 * RenderEffect objects whose get_type() functions return the same.
 */
int RenderEffect::
