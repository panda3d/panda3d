...
/**
 * Removes the indicated NodePath from the collection.  Returns true if the path
 * was removed, false if it was not a member of the collection.
 */
...
/**
 * Removes any duplicate entries of the same NodePaths on this collection.  If a
 * NodePath appears multiple times, the first appearance is retained; subsequent
...
/**
 * Returns the nth NodePath in the collection.  This is the same as get_path(),
 ...
 * and below.  This is the same thing as node()->get_net_collide_mask().  If you
 * want to return what the into_collide_mask of this node itself is, without
 * regard to its children, use node()->get_into_collide_mask().
 */
 ...
 * all nodes at this level and below.  The default is to change all bits, but if
 * bits_to_change is not all bits on, then only the bits that are set in
 * bits_to_change are modified, allowing this call to change only a subset of
 * the bits in the subgraph.
 */
void NodePathCollection::
...
/**
 * Calculates the minimum and maximum vertices of all Geoms at these NodePath's
 * bottom nodes and below This is a tight bounding box; it will generally be
 * tighter than the bounding volume returned by get_bounds() (but it is more
 * expensive to compute).  The return value is true if any points are within the
 ...
 * the default texture stage.  This is the deprecated single-texture variant of
 * this method; it is now superceded by set_texture() that accepts a stage and
 * texture.  However, this method may be used in the presence of multitexture if
 * you just want to adjust the default stage.
 */
void NodePathCollection::
...
/**
 * Applies color scales to all NodePaths in the collection.  The existing color
 * scale is replaced.
 */
void NodePathCollection::
...
/**
 * Applies color scales to all NodePaths in the collection.  The existing color
 * scale, if any, is multiplied by the specified color scale.
 */
void NodePathCollection::
 ...
 * effort is made to apply the attrib to many NodePaths as quickly as possible;
 * redundant RenderState compositions are not duplicated.
 */
void NodePathCollection::
