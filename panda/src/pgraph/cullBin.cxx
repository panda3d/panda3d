 ...
 * subset of the data from this CullBin object that is worth keeping around for
 * next frame.  If a particular CullBin object has no data worth preserving till
 * next frame, it is acceptable to return NULL (which is the default behavior of
 * this method).
 */
PT(CullBin) CullBin::
make_next() const {
 ...
 * represent the various geom objects discovered by the cull.  This is useful
 * mainly for high-level debugging and abstraction tools; it should not be
 * mistaken for the low-level cull result itself.  For the low-level cull
 * result, use draw() to efficiently draw the culled scene.
 */
