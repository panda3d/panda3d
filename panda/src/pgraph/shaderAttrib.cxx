...
/**
 * Sets the geometry instance count.  Do not confuse this with instanceTo, which
 * is used for animation instancing, and has nothing to do with this.  A value
...
/**
 * Returns the ShaderInput as a nodepath.  Assertion fails if there is none, or
 * if it is not a nodepath.
 */
...
/**
 * Returns the ShaderInput as a vector.  Assertion fails if there is none, or if
 * it is not a vector.
 */
...
/**
 * Returns the ShaderInput as a ShaderPtrData struct.  Assertion fails if there
 * is none.  or if it is not a PTA(double/float)
 */
 ...
 * if it is not a texture.  If sampler is not NULL, the sampler state to use for
 * this texture is assigned to it.
 */
...
/**
 * Returns the ShaderInput as a matrix.  Assertion fails if there is none, or if
 * it is not a matrix or NodePath.
 */
...
/**
 * Returns the shader object associated with the node.  If get_override returns
 * true, but get_shader returns NULL, that means that this attribute should
 * disable the shader.
 */
 ...
 * This should return 0 if the two ShaderAttrib objects are equivalent, a number
 ...
 * ShaderAttrib objects whose get_type() functions return the same.
 */
int ShaderAttrib::
