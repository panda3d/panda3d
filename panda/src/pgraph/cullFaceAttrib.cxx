 ...
 * By Panda convention, vertices are ordered counterclockwise when seen from the
 * front, so the M_cull_clockwise will cull backfacing polygons.
 * M_cull_unchanged is an identity attrib; if this is applied to vertices
 * without any other intervening attrib, it is the same as applying the default
 * attrib.
 */
...
/**
 * Constructs a new CullFaceAttrib object that reverses the effects of any other
 * CullFaceAttrib objects in the scene graph.  M_cull_clockwise will be treated
 * as M_cull_counter_clockwise, and vice-versa.  M_cull_none is unchanged.
 */
CPT(RenderAttrib) CullFaceAttrib::
make_reverse() {
 ...
 * mode, unless the reverse flag is set, which swaps CW for CCW and vice-versa.
 * Also, M_cull_unchanged is mapped to M_cull_none.
 */
...
/**
 * Intended to be overridden by derived CullFaceAttrib types to return a unique
 * number indicating whether this CullFaceAttrib is equivalent to the other one.
 ...
 * CullFaceAttrib objects whose get_type() functions return the same.
 */
int CullFaceAttrib::
 ...
 * CullFaceAttrib is encountered in the Bam file.  It should create the
