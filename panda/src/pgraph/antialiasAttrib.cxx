...
/**
 * Constructs a new AntialiasAttrib object.  The mode should be either M_none,
 * M_auto, or a union of any or all of M_point, M_line, M_polygon, and
 * M_multisample.  Also, in addition to the above choices, it may include either
 * of M_better of M_faster to specify a performance/quality tradeoff hint.  If
 * M_none is specified, no antialiasing is performed.  If M_multisample is
 * specified, it means to use the special framebuffer multisample bits for
 * antialiasing, if it is available.  If so, the M_point, M_line, and M_polygon
 * modes are ignored.  This advanced antialiasing mode is only available on
 * certain graphics hardware.  If it is not available, the M_multisample bit is
 * ignored (and the other modes may be used instead, if specified).  M_point,
 * M_line, and/or M_polygon specify per-primitive smoothing.  When enabled,
 * M_point and M_line may force transparency on.  M_polygon requires a frame
 * buffer that includes an alpha channel, and it works best if the primitives
 * are sorted front-to-back.  If M_auto is specified, M_multisample is selected
 * if it is available, otherwise M_polygon is selected, unless drawing lines or
 * points, in which case M_line or M_point is selected (these two generally
 * produce better results than M_multisample)
 */
...
/**
 * Intended to be overridden by derived AntialiasAttrib types to return a unique
 * number indicating whether this AntialiasAttrib is equivalent to the other
 * one.  This should return 0 if the two AntialiasAttrib objects are equivalent,
 ...
 * AntialiasAttrib objects whose get_type() functions return the same.
 */
int AntialiasAttrib::
 ...
 * AntialiasAttrib is encountered in the Bam file.  It should create the
