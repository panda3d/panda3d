...
/**
 * Attempts to open the indicated stream for reading.  The filename is just for
 * information purposes only.  Returns true if successful, false on error.
 */
bool BamFile::
 ...
 * to differentiate these two cases.  The pointers returned by this method will
 * not be valid for use until resolve() is subsequently called.
 */
 ...
 * objects read and make all the pointers valid.  It returns true if all objects
 * are successfully resolved, or false if some have not been (in which case you
 * must call resolve() again later).
 */
bool BamFile::
resolve() {
  if (_reader == (BamReader *)NULL) {
...
/**
 * Although the bam file format is general enough to store a list of objects of
 * arbitrary type, bam files on disk usually contain just one object, a
 ...
 * objects", to differentiate them from the normal scene graph type file.)  This
 * is a convenience method for when you believe you are reading a scene graph
 * bam file.  It reads the one PandaNode and returns it.  It also calls
 * resolve() to fully resolve the object, since we expect this will be the only
 * object in the file.  If the bam file contains something other than a
...
/**
 * Attempts to open the indicated file for writing.  If another file by the same
 * name already exists, it will be silently removed.  Returns true if
...
/**
 * Attempts to open the indicated stream for writing.  The filename is just for
 * information purposes only.  Returns true if successful, false on error.
 */
bool BamFile::
 ...
 * system current major version number if no file is currently open for reading.
 */
int BamFile::
get_file_major_ver() {
  if (_reader == (BamReader *)NULL) {
 ...
 * system current minor version number if no file is currently open for reading.
 */
int BamFile::
get_file_minor_ver() {
  if (_reader == (BamReader *)NULL) {
...
/**
 * Returns the system current major version number.  This is the version number
 * that will be assigned to any generated Bam files.
 */
int BamFile::
...
/**
 * Returns the system current minor version number.  This is the version number
 * that will be assigned to any generated Bam files.
 */
int BamFile::
...
/**
 * Returns the BamReader in charge of performing the read operations.  This will
 * return NULL unless open_read() was called.
 */
BamReader *BamFile::
get_reader() {
  return _reader;
