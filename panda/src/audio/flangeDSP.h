...
/**
 * DSP filter that produces a flange effect on the sound.
 */
class EXPCL_PANDA_AUDIO FlangeDSP : public DSP {
PUBLISHED:
  INLINE FlangeDSP(float mix = 50, float depth = 1, float rate = 0.1);
...
public:
  ...
  static void init_type() {
    DSP::init_type();
    register_type(_type_handle, "FlangeDSP",
                  DSP::get_class_type());
  }
