 ...
 * between any two points in parametric time is proportional to the approximate
 * distance of those same two points along the XYZ curve.  segments_per_unit
 * represents the number of segments to take per each unit of parametric time of
 * the original XYZ curve.  The new timewarp curve (and thus, the apparent range
 ...
 * direction of travel along the XYZ curve, based on the XYZ curve's tangent at
 * each point.
 */
void ParametricCurveCollection::
...
/**
 * Adjusts the apparent length of the curve by applying a new timewarp that maps
 * the range [0..max_t] to the range [0..get_max_t()].  After this call, the
 * curve collection will contain one more timewarp curve, and get_max_t() will
 * return the given max_t value.
 */
void ParametricCurveCollection::
 ...
 * front.  Returns true if the point is valid (i.e.  t is within the bounds
 * indicated by all the timewarp curves and within the bounds of the curves
 * themselves), or false otherwise.
 */
bool ParametricCurveCollection::
 ...
 * indicated by the first XYZ curve in the collection and the rotation indicated
 * by the first HPR curve in the collection, after t has been modified by all
 * the timewarp curves in the collection applied in sequence, from back to
 * front.  Returns true if the point is valid (i.e.  t is within the bounds
 * indicated by all the timewarp curves and within the bounds of the curves
 * themselves), or false otherwise.
 */
...
/**
 * Adjust the XYZ curve at the indicated time to the new value.  The curve shape
 * will change correspondingly.  Returns true if successful, false if unable to
 * make the adjustment for some reason.
 */
bool ParametricCurveCollection::
...
/**
 * Adjust the HPR curve at the indicated time to the new value.  The curve shape
 * will change correspondingly.  Returns true if successful, false if unable to
 * make the adjustment for some reason.
 */
bool ParametricCurveCollection::
...
/**
 * Ensures all the curves are freshly computed and up-to-date.  Returns true if
 * everything is valid, false if at least one curve is incorrect.
 */
bool ParametricCurveCollection::
recompute() {
  bool all_ok = true;
 ...
 * end with the second one.  Either a or b may be the same as 'this'.  This will
 * lose any timewarps on the input curves.  Returns true if successful, false on
 * failure.
 */
bool ParametricCurveCollection::
...
/**
 * Writes a complete multi-line description of the ParametricCurveCollection to
 ...
 * representation of the curve is kept up to date.  This is called automatically
 * by the ParametricCurveDrawer.  Any number of Drawers may be registered with a
...
/**
 * Computes the orientation at the given point in time, based on the tangent of
 * the XYZ curve.  Returns true if the orientation can be determined, or false
 * if it cannot (in which case hpr is left unchanged).
 */
bool ParametricCurveCollection::
...
/**
 * Unregisters the curve with the list of drawers that share this collection, in
 * preparation for removing it from the _curves list.
 */
void ParametricCurveCollection::
