 ...
 * parametric time within the indicated curve segment.  t should be in the range
 * [0, 1].  The curve is internally represented as a number of connected (or
 * possibly unconnected) piecewise continuous segments.  The exact number of
 * segments for a particular curve depends on the knot vector, and is returned
 * by get_num_segments().  Normally, eval_point() is used to evaluate a point
...
/**
 * As eval_segment_point, but computes the tangent to the curve at the indicated
 * point.  The tangent vector will not necessarily be normalized, and could be
...
/**
 * Evaluates the curve in n-dimensional space according to the extended vertices
 * associated with the curve in the indicated dimension.
 */
...
/**
 * Determines the set of subdivisions necessary to approximate the curve with a
 * set of linear segments, no point of which is farther than tolerance units
 * from the actual curve.  After this call, you may walk through the resulting
 * set of samples with get_num_samples(), get_sample_t(), and
 * get_sample_point().
 */
void NurbsCurveResult::
