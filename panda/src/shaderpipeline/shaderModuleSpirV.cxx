  ...
  _instructions(std::move(words))
{
  ...
  SpirVModule &module = transformer.get_module();
  ...
  Id type_id = module.find_named("$Global");
  if (type_id) {
  ...
  // Compute how the variables are used, for the extraction below.  The type
  // replacements made inside the loop do not affect the usage information.
  ...
  for (uint32_t word = 0; word < transformer.get_id_bound(); ++word) {
    Id id(word);
    SpirVModule::DefinitionType dtype = module.get_definition_type(id);

    if ((dtype == SpirVModule::DT_variable || dtype == SpirVModule::DT_function_parameter) &&
        usage.is_used(id)) {
      const ShaderType *used_type = module.resolve_type(id);
      if (used_type != nullptr) {
        if (used_type->contains_scalar_type(ShaderType::ST_double)) {
          _used_caps |= C_double;
        }
        if (const ShaderType::Matrix *matrix_type = used_type->as_matrix()) {
          if (matrix_type->get_num_rows() != matrix_type->get_num_columns()) {
            _used_caps |= C_non_square_matrices;
          }
        }
        if (used_type->contains_scalar_type(ShaderType::ST_uint)) {
          _used_caps |= C_unified_model;
        }
      }
    }

    if (dtype == SpirVModule::DT_variable && module.get_function_id(id) == 0 &&
        module.get_builtin(id) == spv::BuiltInMax) {
      const ShaderType *var_type = module.resolve_type(id);
      if (var_type == nullptr) {
        continue;
      }
      // Ignore empty structs/arrays.
      int num_locations = var_type->get_num_interface_locations();
      if (num_locations == 0) {
        continue;
      }

      spv::StorageClass storage_class = module.get_storage_class(id);

      Variable var;
      var.type = var_type;
      var.name = InternalName::make(module.get_name(id));
      var._location = module.get_location(id);
      var.id = id;
      ...
      if (storage_class == spv::StorageClassInput) {
        ...
        if (stage == Stage::FRAGMENT) {
          ...
          if (var_type->contains_scalar_type(ShaderType::ST_uint) ||
              var_type->contains_scalar_type(ShaderType::ST_int) ||
              var_type->contains_scalar_type(ShaderType::ST_bool)) {
            _used_caps |= C_unified_model;
          }
        }
      }
        ...
        if (sampled_image_type != nullptr) {
          ...
          // properly generate shadow samplers and helps us distinguish between
          // shadow samplers and regular ones.
          if (usage.is_dref_sampled(id) && !sampled_image_type->is_shadow()) {
        ...
        if (usage.is_dynamically_indexed(id) &&
            (sampled_image_type != nullptr || var_type->contains_opaque_type())) {
          _used_caps |= C_dynamic_indexing;
        }
        _parameters.push_back(std::move(var));
      }
      else if (storage_class == spv::StorageClassStorageBuffer) {
        // For whatever reason, in GLSL, the name of an SSBO is derived from the
        // name of the struct type.  This may be an array of structs, so unwrap
        // any array layers.
        Id struct_type_id = module.unwrap_pointer_type(module.get_type_id(id));
        const SpirVModule::Instruction *struct_decl = module.find_declaration(struct_type_id);
        while (struct_decl != nullptr &&
               (struct_decl->opcode == spv::OpTypeArray ||
                struct_decl->opcode == spv::OpTypeRuntimeArray)) {
          struct_type_id = Id(struct_decl->args[1]);
          struct_decl = module.find_declaration(struct_type_id);
        }
        nassertd(struct_decl != nullptr && struct_decl->opcode == spv::OpTypeStruct) continue;

        const std::string &struct_name = module.get_name(struct_type_id);
        nassertd(!struct_name.empty()) continue;

        var.name = InternalName::make(struct_name);
        _parameters.push_back(std::move(var));

        _used_caps |= C_storage_buffer;
      }
    }
    else if (dtype == SpirVModule::DT_variable && module.get_function_id(id) == 0 &&
             usage.is_used(id) &&
             module.get_storage_class(id) == spv::StorageClassInput) {
      // Built-in input variable.
      switch (module.get_builtin(id)) {
      ...
      case spv::BuiltInFragCoord:
        ...
        {
          const ShaderType *builtin_type = module.resolve_type(id);
          const ShaderType::Vector *vector_type = builtin_type->as_vector();
          if (vector_type != nullptr || builtin_type->as_scalar() != nullptr) {
            if (vector_type == nullptr ||
              ...
              ShaderType::ScalarType scalar_type =
                ...
                : builtin_type->as_scalar()->get_scalar_type();
    ...
    else if (dtype == SpirVModule::DT_type) {
      const ShaderType *type = module.resolve_type(id);
      if (type != nullptr) {
        if (const ShaderType::Matrix *matrix_type = type->as_matrix()) {
          if (matrix_type->get_num_rows() != matrix_type->get_num_columns()) {
            _used_caps |= C_non_square_matrices;
          }
        }
    ...
    else if (dtype == SpirVModule::DT_spec_constant) {
      const ShaderType *type = module.resolve_type(id);
      if (type != nullptr) {
        SpecializationConstant spec_constant;
        spec_constant.id = module.get_spec_id(id);
        spec_constant.name = InternalName::make(module.get_name(id));
        spec_constant.type = type;
        if (shader_cat.is_debug()) {
          shader_cat.debug()
            << "Found specialization constant " << module.get_name(id)
            << " with type " << *type << " and ID " << spec_constant.id << "\n";
        }
 ...
 */
void ShaderModuleSpirV::
post_analyze(const SpirVModule &module, const SpirVUsageAnalysis &usage) {
#ifndef NDEBUG
  ...
  {
    pmap<Id, const ShaderType::Struct *> struct_types;
    for (Variable &var : _parameters) {
      if (module.get_storage_class(Id(var.id)) == spv::StorageClassUniformConstant) {
        const ShaderType *type = module.resolve_type(Id(var.id));
        if (type != nullptr && type->is_aggregate_type()) {
          // Store all the uniform struct types while we have them, as a
          // convenience for the GL back-end, which may need them.
          module.collect_nested_structs(struct_types, module.get_type_id(Id(var.id)));
        }
      }
    }
    _uniform_struct_types.insert(struct_types.begin(), struct_types.end());
  }

  // Look up the extended instruction sets we care about below.
  pset<uint32_t> glsl_std_450_imports;
  pset<uint32_t> debug_printf_imports;
  for (size_t i = 0; i < module.get_num_ext_inst_imports(); ++i) {
    const SpirVModule::Instruction &import = module.get_ext_inst_import(i);
    std::string name = import.get_string(1);
    if (name == "GLSL.std.450") {
      glsl_std_450_imports.insert(import.args[0]);
    }
    else if (name == "NonSemantic.DebugPrintf") {
      debug_printf_imports.insert(import.args[0]);
    }
  }
  ...
  for (InstructionIterator it = _instructions.begin_annotations(); it != _instructions.end_annotations(); ++it) {
    ...
    if (op.opcode == spv::OpDecorate) {
      if (module.get_builtin(Id(op.args[0])) != spv::BuiltInMax) {
        continue;
      }
  ...
  for (InstructionIterator it = _instructions.begin_functions(); it != _instructions.end(); ++it) {
    ...
    case spv::OpExtInst:
      {
        if (glsl_std_450_imports.count(op.args[2]) && op.args[3] == GLSLstd450RoundEven) {
          // We mark the use of the GLSL roundEven() function, which requires
          // GLSL 1.30 or HLSL SM 4.0.
          _used_caps |= C_unified_model;
        }
        if (debug_printf_imports.count(op.args[2]) && op.args[3] == 1) {
          _emulatable_caps |= C_debug_output;
          if (op.nargs >= 5) {
            // This is actually a special way to mark an assertion failure.
            if (module.get_definition_type(Id(op.args[4])) == SpirVModule::DT_string &&
                module.resolve_string(Id(op.args[4])) == "%!") {
              ++_assert_count;
            }
          }
        }
    ...
    case spv::OpImageSampleProjDrefImplicitLod:
      ...
      {
        const ShaderType *sampler_type = module.resolve_type(Id(op.args[2]));
        if (sampler_type != nullptr) {
          const ShaderType::SampledImage *sampler = sampler_type->as_sampled_image();
          if (sampler != nullptr &&
    ...
    case spv::OpImageQueryLevels:
      {
        ...
        } else if (usage.is_sampled_image_value(Id(op.args[2]))) {
        ...
        Id var_id = usage.get_origin(Id(op.args[2]));
        if (var_id != 0) {
          if (!usage.is_dynamically_indexed(var_id)) {
            if (op.opcode != spv::OpImageQuerySizeLod ||
                (module.get_definition_type(Id(op.args[3])) == SpirVModule::DT_constant &&
    ...
    case spv::OpImageSparseGather:
      {
        if (module.get_definition_type(Id(op.args[4])) == SpirVModule::DT_constant &&
            module.resolve_constant(Id(op.args[4])) == 0) {
...
void ShaderModuleSpirV::
finalize(BamReader *) {
  SpirVModule module(_instructions);
  post_analyze(module, module.analyze_usage());
}
