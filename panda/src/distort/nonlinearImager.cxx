...
~NonlinearImager() {
  ...
  if (_recompute_task != (AsyncTask *)nullptr) {
...
add_screen(const NodePath &screen, const string &name) {
  ...
  new_screen._buffer = (GraphicsOutput *)nullptr;
...
get_buffer(int index) const {
  nassertr(index >= 0 && index < (int)_screens.size(), (GraphicsOutput *)nullptr);
...
set_texture_size(int index, int width, int height) {
  ...
  if (screen._buffer != (GraphicsOutput *)nullptr) {
    bool removed = _engine->remove_window(screen._buffer);
    screen._buffer = (GraphicsOutput *)nullptr;
    nassertv(removed);
  }
...
add_viewer(DisplayRegion *dr) {
  ...
  nassertr(window != (GraphicsOutput *)nullptr, -1);

  GraphicsStateGuardian *gsg = window->get_gsg();
  nassertr(gsg != (GraphicsStateGuardian *)nullptr, -1);

  GraphicsEngine *engine = gsg->get_engine();
  nassertr(engine != (GraphicsEngine *)nullptr, -1);
  ...
  if (viewer._viewer.is_empty()) {
    viewer._viewer_node = (LensNode *)nullptr;
  } else {
...
get_viewer(int index) const {
  nassertr(index >= 0 && index < (int)_viewers.size(), (DisplayRegion *)nullptr);
...
recompute() {
  ...
  for (vi = 0; vi < _viewers.size(); ++vi) {
    ...
    if (viewer._viewer_node != (LensNode *)nullptr &&
        viewer._viewer_node->get_lens() != (Lens *)nullptr) {
