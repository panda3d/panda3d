...
/**
 * This version of this method is deprecated and will soon be removed.  Use the
 * version that takes two parameters instead.
 */
int NonlinearImager::
...
/**
 * Adds a new ProjectionScreen to the list of screens that will be processed by
 * the NonlinearImager.  Each ProjectionScreen represents a view into the world.
 * It must be based on a linear camera (or whatever kind of camera is respected
 * by the graphics engine).  Each ProjectionScreen object should already have
 * some screen geometry created.  As each frame is rendered, an offscreen image
 * will be rendered from the source camera associated with each
 * ProjectionScreen, and the resulting image will be applied to the screen
 * geometry.  The return value is the index number of the new screen.
 */
int NonlinearImager::
 ...
 * indicated screen.  This must be less than or equal to the window size, and it
 * should be a power of two.  In general, the larger the texture, the greater
 * the detail of the rendered scene.
 */
void NonlinearImager::
 ...
 * particular screen.  The parameter must be a NodePath whose node is a Camera.
 * The camera itself indicates the scene that is to be rendered.
 */
void NonlinearImager::
 ...
 * like a fisheye or cylindrical lens.  This sets up a special scene graph for
 * this DisplayRegion alone and sets up the DisplayRegion with a specialty
 * camera.  If future changes to the camera are desired, you should use the
 * set_viewer_camera() interface.  All viewers must share the same
 * GraphicsEngine.  The return value is the index of the new viewer.
 */
int NonlinearImager::
 ...
 * will be assigned to the geometry within the NonlinearImager.  It is not
 * necessary to call this except to change the camera after a viewer has been
 * added, since the default is to use whatever camera is associated with the
 * DisplayRegion at the time the viewer is added.  The NodePath must refer to a
 * LensNode (or a Camera).
 */
void NonlinearImager::
...
/**
 * Returns the NodePath to the LensNode that is to serve as nth viewer for this
 * screen.
 */
NodePath NonlinearImager::
 ...
 * This is the scene graph in which the screen meshes within the dark room have
 * been flattened into the appropriate transformation according to the viewer's
 * lens properties (and position relative to the screens).  It is this scene
 * graph that is finally rendered to the window.
 */
NodePath NonlinearImager::
 ...
 * in which all of the ProjectionScreens and the viewer cameras reside.  It's a
 * standalone scene with a few projection screens arranged artfully around one
 * or more viewers; it's so named because it's a little virtual theater.
