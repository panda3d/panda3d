...
/**
 * A ProjectionScreen implements a simple system for projective texturing.  The
 * ProjectionScreen node is the parent of a hierarchy of geometry that is
 * considered a "screen"; the ProjectionScreen will automatically recompute all
 * the UV's (for a particular texture stage) on its subordinate geometry
 * according to the relative position and lens parameters of the indicated
 * LensNode.  All this does is recompute UV's; the caller is responsible for
 * applying the appropriate texture(s) to the geometry.  This does not take
 * advantage of any hardware-assisted projective texturing; all of the UV's are
 * computed in the CPU.  (Use NodePath::project_texture() to enable hardware-
 * assisted projective texturing.)  However, the ProjectionScreen interface does
 * support any kind of lens, linear or nonlinear, that might be defined using
 * the Lens interface, including fisheye and cylindrical lenses.
 */
