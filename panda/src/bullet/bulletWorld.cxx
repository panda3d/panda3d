...
/**
 * Attaches a single constraint to a world.  Collision checks between the linked
 * objects will be disabled if the second parameter is set to TRUE.
 */
void BulletWorld::
...
/**
 * Performas a test for all bodies which are currently in contact with the given
 * body.  The test returns a BulletContactResult object which may contain zero,
 * one or more contacts.  If the optional parameter use_filter is set to TRUE
 * this test will consider filter settings.  Otherwise all objects in contact
 * are reported, no matter if they would collide or not.
 */
 ...
 * The test returns a BulletContactResult object which may contain zero or one
 * contacts.
 */
BulletContactResult BulletWorld::
