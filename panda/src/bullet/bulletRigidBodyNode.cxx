...
/**
 * Hook which should be called whenever the total shape of a body changed.  Used
...
/**
 * Returns the total mass of a rigid body.  A value of zero means that the body
 * is staic, i.e.  has an infinite mass.
 */
 ...
 * possible that automatic calculation of intertia is trigger after calling this
 * method, and thus overwriting the explicitly set value again.  This happens
 * when: (a) the mass is set after the inertia.  (b) a shape is added or removed
 * from the body.  (c) the scale of the body changed.
 */
void BulletRigidBodyNode::
...
/**
 * Returns the inertia of the rigid body.  Inertia is given as a three component
 * vector.  A component value of zero means infinite inertia along this
 * direction.
 */
 ...
 * from BulletRigidBodyNode::transform_changed(). For kinematic bodies the
 * global transform is required since Bullet queries the body transform via
 * MotionState::getGlobalStranform(). For dynamic bodies the global scale is
 * required, since Bullet will overwrite the member _trans by calling
