...
/**
 * Specifies the maximum number of frames that a sub-thread will attempt to read
 * ahead of the current frame.  Setting this to a nonzero allows the video
 * decoding to take place in a sub-thread, which smoothes out the video decoding
 * time by spreading it evenly over several frames.  Set this number larger to
 * increase the buffer between the currently visible frame and the first
 * undecoded frame; set it smaller to reduce memory consumption.  Setting this
 * to zero forces the video to be decoded in the main thread.  If threading is
 * not available in the Panda build, this value is always zero.
 */
void FfmpegVideoCursor::
...
/**
 * Explicitly stops the ffmpeg decoding thread.  There is normally no reason to
 * do this unless you want to maintain precise control over what threads are
 * consuming CPU resources.  Calling this method will make the video update in
 * the main thread, regardless of the setting of max_readahead_frames, until you
...
/**
 * Returns true if the thread has been started, false if not.  This will always
 * return false if max_readahead_frames is 0.
 */
...
/**
 * Called within the sub-thread.  Assumes the lock is already held.  If there is
 * something for the thread to do, does it and returns true.  If there is
...
/**
 * Called within the sub-thread.  Slides forward until the indicated frame, then
 * fetches a frame from the stream and stores it in the frame buffer.  Sets
 * _begin_frame and _end_frame to indicate the extents of the frame.  Sets
 * _frame_ready true to indicate a frame is now available, or false if it is not
 * (for instance, because the end of the video was reached).
 */
void FfmpegVideoCursor::
...
/**
 * Called within the sub-thread.  Decodes the data in the specified packet into
 * _frame.
 */
void FfmpegVideoCursor::
...
/**
 * As above, with the ffmpeg global lock held (if configured on).  Also only if
 * ffmpeg-support-seek is on.
 */
void FfmpegVideoCursor::
...
/**
 * Casts about within the stream for a reasonably-close frame to seek to.  We're
 * trying to get as close as possible to target_frame.
 */
int FfmpegVideoCursor::
...
/**
 * Called within the sub-thread.  Exports the contents of the frame buffer into
 * the indicated target buffer.
 */
void FfmpegVideoCursor::
