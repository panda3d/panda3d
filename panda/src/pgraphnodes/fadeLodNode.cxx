...
cull_callback(CullTraverser *trav, CullTraverserData &data) {
  ...
  if (ldata == (AuxSceneData *)nullptr || now > ldata->get_expiration_time()) {
...
get_fade_1_old_state() {
  if (_fade_1_old_state == (const RenderState *)nullptr) {
...
get_fade_1_new_state(PN_stdfloat in_alpha) {
  if (_fade_1_new_state == (const RenderState *)nullptr) {
...
get_fade_2_old_state(PN_stdfloat out_alpha) {
  if (_fade_2_old_state == (const RenderState *)nullptr) {
...
get_fade_2_new_state() {
  if (_fade_2_new_state == (const RenderState *)nullptr) {
