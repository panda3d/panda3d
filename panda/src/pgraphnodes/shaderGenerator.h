 ...
 * fixed function pipeline supports features like normal mapping, gloss mapping,
 * cartoon lighting, and so forth.  It works by automatically generating a
 * shader from a given RenderState.  Currently, there is one ShaderGenerator
 * object per GraphicsStateGuardian.  It is our intent that in time, people will
 * write classes that derive from ShaderGenerator but which yield slightly
 * different results.  The ShaderGenerator owes its existence to the 'Bamboo
 * Team' at Carnegie Mellon's Entertainment Technology Center.  This is a group
 * of students who, as a semester project, decided that next-gen graphics should
 * be accessible to everyone, even if they don't know shader programming.  The
 * group consisted of:  Aaron Lo, Programmer Heegun Lee, Programmer Erin
 * Fernandez, Artist/Tester Joe Grubb, Artist/Tester Ivan Ortega, Technical
 * Artist/Tester  Thanks to them!
 */
