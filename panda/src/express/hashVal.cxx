 ...
 * false if the file cannot be read.  This method is only defined if we have the
 * OpenSSL library (which provides md5 functionality) available.
 */
bool HashVal::
...
#ifdef HAVE_OPENSSL
/**
 * Generates the hash value by hashing the indicated data.  This method is only
 * defined if we have the OpenSSL library (which provides md5 functionality)
 * available.
 */
void HashVal::
...
/**
 * Encodes the indicated unsigned int into an eight-digit hex string, stored at
 * the indicated buffer and the following 8 positions.
 */
void HashVal::
