...
/**
 * Attempts to delete this file or directory.  This can remove a single file or
 * an empty directory.  It will not remove a nonempty directory.  Returns true
 ...
 * new filename (but not a directory).  If the original file is a directory, the
 * new filename must not already exist.  If the file is a directory, the new
 * filename must be within the same mount point.  If the file is an ordinary
 * file, the new filename may be anywhere; but if it is not within the same
 * mount point then the rename operation is automatically performed as a two-
 * step copy-and-delete operation.
 */
bool VirtualFileSimple::
 ...
 * on failure.  If auto_wrap is true, an explicitly-named .pz file is
 * automatically compressed while writing.  If truncate is true, the file is
 * truncated to zero length before writing.
 */
 ...
 * excluding those whose basenames are listed in mount_points.  Returns true if
 * successful, false if the file is not a directory or the directory cannot be
 * read.
 */
bool VirtualFileSimple::
