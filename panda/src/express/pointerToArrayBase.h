 ...
 * internally by a PointerToArray.  It is basically a NodeReferenceCount-capable
 * STL vector.  We use NodeReferenceCount (instead of just ReferenceCount),
 * which adds node_ref() and node_unref() to the standard ref() and unref().
 * This is particularly useful for GeomVertexArrayData; other classes may or may
 * not find this additional counter useful, but since it adds relatively little
 * overhead (compared with what is presumably a largish array), we go ahead and
 * add it here, even though it is inherited by many different parts of the
...
/**
 * This is the base class for PointerToArray and ConstPointerToArray.  Don't try
 * to use it directly; use either derived class instead.  This extends
 * PointerToBase to be a pointer to a ReferenceCountedVector, above, which is
 * essentially a reference-counted STL vector.
 */
template <class Element>
