...
/**
 * Opens up the image file and tries to read its header information to determine
 * its size, number of channels, etc.  If successful, updates the header
...
/**
 * Reads the image header information only from the indicated stream.  The
 * filename is advisory only, and may be used to suggest a type if it has a
 * known extension.  If type is non-NULL, it is a suggestion for the type of
 * file it is (and a non-NULL type will override any magic number test or
 * filename extension lookup).  Returns true if successful, false on error.
 */
bool PNMImageHeader::
...
/**
 * Returns a newly-allocated PNMReader of the suitable type for reading from the
 * indicated image filename, or NULL if the filename cannot be read for some
 * reason.  The filename "-" always stands for standard input.  If type is
 * specified, it is a suggestion for the file type to use.  The PNMReader should
...
/**
 * Returns a newly-allocated PNMReader of the suitable type for reading from the
 * already-opened image file, or NULL if the file cannot be read for some
 * reason.  owns_file should be set true if the PNMReader is to be considered
 * the owner of the stream pointer (in which case the stream will be deleted on
 * completion, whether successful or not), or false if it should not delete it.
 ...
 * file type.  If magic_number is nonempty, it is assumed to represent the first
 * few bytes that have already been read from the file.  Some file types may
 * have difficulty if this is more than two bytes.  If type is non-NULL, it is a
 * suggestion for the file type to use.  The PNMReader should be deleted when it
 * is no longer needed.
 */
PNMReader *PNMImageHeader::
...
/**
 * Returns a newly-allocated PNMWriter of the suitable type for writing an image
 * to the indicated filename, or NULL if the filename cannot be written for some
 * reason.  The filename "-" always stands for standard output.  If type is
 * specified, it is a suggestion for the file type to use.  The PNMWriter should
 * be deleted when it is no longer needed.
 */
 ...
 * reason.  owns_file should be set true if the PNMWriter is to be considered
 * the owner of the stream pointer (in which case the stream will be deleted on
 * completion, whether successful or not), or false if it should not delete it.
 * The filename parameter is optional here, since the file has already been
 * opened; it is only used to examine the extension and attempt to guess the
 * intended file type.  If type is non-NULL, it is a suggestion for the file
 * type to use.  The PNMWriter should be deleted when it is no longer needed.
 */
PNMWriter *PNMImageHeader::
 ...
 * bytes already extracted.  Returns true if successful, false if an end of file
 * or error occurred before num_bytes could be read.
 */
bool PNMImageHeader::
...
/**
 * Computes a histogram of the colors used in the indicated rgb/grayscale array
 * and/or alpha array.  This is most likely to be useful in a PNMWriter class,
 * but it is defined at this level in case it has general utilty for PNMImages.
 * Also see PNMImage::make_histogram(), which is a higher-level function.  The
 * max_colors parameter, if greater than zero, limits the maximum number of
 * colors we are interested in.  If we encounter more than this number of
