...
/**
 * The constructor stores a pointer to the PNMImage you pass it, but it does not
 * take ownership of the object; you are responsible for ensuring that the
 * PNMImage does not destruct during the lifetime of the PNMPainter object.  The
 * xo, yo coordinates specify an optional offset for fill coordinates.  If you
 * are painting with a pattern fill, these specify the virtual coordinates of
 * the upper-left corner of the image, which can allow you to adjust the pattern
 * to line up with nested images, if necessary.
 */
PNMPainter::
 ...
 * outline, and the current fill brush for the interior.  The two coordinates
 * specify any two diagonally opposite corners.
 */
void PNMPainter::
