...
/**
 * This flavor of clear() reinitializes the image to an empty (black) image with
 ...
 * must be the same size.  If right_shift is negative, it means a left shift.
 */
void PNMImage::
...
/**
 * Copies just the header information into this image.  This will blow away any
 * image data stored in the image.  The new image data will be allocated, but
 * left unitialized.
 */
void PNMImage::
...
/**
 * Reads the indicated image filename.  If type is non-NULL, it is a suggestion
 * for the type of file it is.  Returns true if successful, false on error.
 */
bool PNMImage::
...
/**
 * Reads the image data from the indicated stream.  The filename is advisory
 * only, and may be used to suggest a type if it has a known extension.  If type
 * is non-NULL, it is a suggestion for the type of file it is (and a non-NULL
 * type will override any magic number test or filename extension lookup).
 ...
 * determine its size) before actually reading the entire image.  The PNMReader
 * is always deleted upon completion, whether successful or not.
 */
bool PNMImage::
...
/**
 * Writes the image to the indicated ostream.  The filename is advisory only,
 * and may be used suggest a type if it has a known extension.  If type is non-
 * NULL, it is a suggestion for the type of image file to write.
 */
bool PNMImage::
...
/**
 * This flavor of write() uses an already-existing PNMWriter to write the image
 * file.  You can get a writer via the PNMImageHeader::make_writer() methods.
 ...
 * original gray level; color images are converted to grayscale according to the
 * value of Bright().  The alpha channel, if added, is initialized to zero.
 */
void PNMImage::
...
/**
 * Converts the colors in the image to the indicated color space.  This may be a
 * lossy operation, in particular when going from sRGB to linear.  The alpha
 * channel remains untouched.  Note that, because functions like get_xel() and
 * set_xel() work on linearized floating-point values, this conversion won't
 * affect those values (aside from some minor discrepancies due to storage
 * precision).  It does affect the values used by get_xel_val() and
 * set_xel_val(), though, since those operate on encoded colors.  Some color
 * spaces, particularly scRGB, may enforce the use of a particular maxval
 * setting.
 */
void PNMImage::
...
/**
 * Converts the image from RGB to grayscale.  Any alpha channel, if present, is
 * left undisturbed.  The optional rc, gc, bc values represent the relative
 ...
 * that pixel's alpha value.  This does not modify any alpha values.
 */
void PNMImage::
premultiply_alpha() {
  if (!has_alpha()) {
 ...
 * and blue components are divided by that pixel's alpha value.  This does not
 * modify any alpha values.
 */
void PNMImage::
unpremultiply_alpha() {
  if (!has_alpha()) {
 ...
 * should be in the range 0..(get_num_channels()-1).  The channels are ordered
 * B, G, R, A.  This is slightly less optimal than setting the component values
 * directly by named methods.  The value given should be in the range 0..maxval.
 */
void PNMImage::
 ...
 * should be in the range 0..(get_num_channels()-1).  The channels are ordered
 * B, G, R, A.  This is slightly less optimal than accessing the component
 * values directly by named methods.  The value returned is a float in the range
 * 0..1.
 */
float PNMImage::
 ...
 * should be in the range 0..(get_num_channels()-1).  The channels are ordered
 * B, G, R, A.  This is slightly less optimal than setting the component values
 * directly by named methods.  The value given should be a float in the range
 * 0..1.
 */
void PNMImage::
...
/**
 * Smoothly blends the indicated pixel value in with whatever was already in the
 * image, based on the given alpha value.  An alpha of 1.0 is fully opaque and
 * completely replaces whatever was there previously; alpha of 0.0 is fully
...
/**
 * Replaces the underlying PNMImage array with the indicated pointer.  Know what
 * you are doing!  The new array must be the correct size and must have been
 * allocated via PANDA_MALLOC_ARRAY().  The PNMImage object becomes the owner of
 * this pointer and will eventually free it with PANDA_FREE_ARRAY().  The
 * previous array, if any, will be freed with PANDA_FREE_ARRAY() when this call
 * is made.
 */
void PNMImage::
set_array(xel *array) {
 ...
 * unconditionally.  If pixel_scale is not 1.0, it specifies an amount to scale
 * each *alpha* value of the source image before applying it to the target
 * image.  If pixel_scale is 1.0 and the copy has no alpha channel, this
 * degenerates into copy_sub_image().
 */
void PNMImage::
 ...
 * of the destination, after scaling by the specified pixel_scale.  Unlike
 * blend_sub_image(), the alpha channel is not treated specially.
 */
void PNMImage::
...
/**
 * Behaves like copy_sub_image(), but the resulting color will be the darker of
 * the source and destination colors at each pixel (and at each R, G, B, A
 * component value).  If pixel_scale is not 1.0, it specifies an amount to scale
 * each pixel value of the source image before applying it to the target image.
 * The scale is applied with the center at 1.0: scaling the pixel value smaller
...
/**
 * Behaves like copy_sub_image(), but the resulting color will be the lighter of
 * the source and destination colors at each pixel (and at each R, G, B, A
 * component value).  If pixel_scale is not 1.0, it specifies an amount to scale
 * each pixel value of the source image before applying it to the target image.
 */
 ...
 * depending on the pixel value of the indicated channel of select_image.  For
 * each pixel (x, y):  s = select_image.get_channel(x, y, channel). Set this
 * image's (x, y) to:  lt.get_xel(x, y) if s < threshold, or  ge.get_xel(x, y)
 * if s >= threshold  Any of select_image, lt, or ge may be the same PNMImge
 * object as this image, or the same as each other; or they may all be
 * different.  All images must be the same size.  As a special case, lt and ge
 * may both be 1x1 images instead of the source image size.
 */
void PNMImage::
...
/**
 * Replaces this image with a grayscale image whose gray channel represents the
 * linear Manhattan distance from the nearest dark pixel in the given mask
 * image, up to the specified radius value (which also becomes the new maxval).
 * radius may range from 0 to maxmaxval; smaller values will compute faster.  A
 * dark pixel is defined as one whose pixel value is < threshold.  If
 * shrink_from_border is true, then the mask image is considered to be
 * surrounded by a border of dark pixels; otherwise, the border isn't
 * considered.  This can be used, in conjunction with threshold, to shrink a
 * mask image inwards by a certain number of pixels.  The mask image may be the
 * same image as this one, in which case it is destructively modified by this
 * process.
 */
void PNMImage::
...
/**
 * Replaces this image with a grayscale image whose gray channel represents the
 * linear Manhattan distance from the nearest white pixel in the given mask
 * image, up to the specified radius value (which also becomes the new maxval).
 * radius may range from 0 to maxmaxval; smaller values will compute faster.  A
 * white pixel is defined as one whose pixel value is >= threshold.  This can be
 * used, in conjunction with threshold, to grow a mask image outwards by a
 * certain number of pixels.  The mask image may be the same image as this one,
 * in which case it is destructively modified by this process.
 */
void PNMImage::
...
/**
 * index_image is a WxH grayscale image, while pixel_values is an Nx1 color (or
 * grayscale) image.  Typically pixel_values will be a 256x1 image.  Fills the
 * PNMImage with a new image the same width and height as index_image, with the
 * same number of channels as pixel_values.  Each pixel of the new image is
 * computed with the formula:  new_image(x, y) = pixel_values(index_image(x,
 * y)[channel], 0)  No interpolation is performed; the nearest value in
 ...
 * min_val are set to 0, and values above max_val are set to 1. Does not affect
 ...
 * The min_radius and max_radius are in the scale 0..1, where 1.0 means the full
 * width of the image.  If min_radius == max_radius, the edge is sharp (but
 * still antialiased); otherwise, the pixels between min_radius and max_radius
 * are smoothly blended between fg and bg colors.
 */
void PNMImage::
 ...
 * pixels are set to the indicated color.  If any of the values is negative,
...
/**
 * Resizes from the indicated image into this one by performing a nearest-point
 * sample.
 */
void PNMImage::
...
/**
 * Fills the image with a grayscale perlin noise pattern based on the indicated
 * parameters.  Uses set_xel to set the grayscale values.  The sx and sy
 * parameters are in multiples of the size of this image.  See also the
 ...
 * grayscale image, the blue_exponent value is used for the grayscale value, and
 * red_exponent and green_exponent are unused.
 */
void PNMImage::
...
/**
 * Sets each pixel value to the sum of the corresponding pixel values in the two
 * given images.  Only valid when both images have the same size.
 */
void PNMImage::
 ...
 * Note that the floating-point values in the 0..1 range are multiplied, not in
 * the 0..maxval range.  Only valid when both images have the same size.
 */
void PNMImage::
operator *= (const PNMImage &other) {
  nassertv(is_valid() && other.is_valid());
