 ...
 * error or end of file.  The x_size and y_size parameters are the value of
 * _x_size and _y_size as originally filled in by the constructor; it is the
 * actual number of pixels in the image.  (The _x_size and _y_size members may
 * have been automatically modified by the time this method is called if we are
 * scaling on load, so should not be used.)
 */
bool PNMReader::
 ...
 * (including pipes, etc.), or false if the reader must occasionally fseek() on
 * its input stream, and thus only disk streams are supported.
 */
bool PNMReader::
 ...
 * size, returned as an exponent of power of 2 (that is, a bit shift).  Only
 * power-of-two reductions are supported, since those are common, easy, and
 * fast.  Other reductions will be handled in the higher level code.
 */
int PNMReader::
