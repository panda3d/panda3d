...
/**
 * Resets to an empty table with a specific size.  The case of num_channels == 0
 * is allowed only in the case that x_size and y_size are also == 0; and this
 * makes an empty (and invalid) PfmFile.
 */
void PfmFile::
...
/**
 * Reads the PFM data from the indicated file, returning true on success, false
 * on failure.  This can also handle reading a standard image file supported by
 * PNMImage; it will be quietly converted to a floating-point type.
 */
bool PfmFile::
 ...
 * false on failure.  This can also handle reading a standard image file
 * supported by PNMImage; it will be quietly converted to a floating-point type.
 */
bool PfmFile::
...
/**
 * Reads the PFM data using the indicated PNMReader.  The PNMReader is always
 * deleted upon completion, whether successful or not.
 */
bool PfmFile::
 ...
 * on failure.  If the type implied by the filename extension supports floating-
 * point, the data will be written directly; otherwise, the floating-point data
 * will be quietly converted to the appropriate integer type.
 */
bool PfmFile::
...
/**
 * Writes the PFM data to the indicated stream, returning true on success, false
 * on failure.
 */
bool PfmFile::
...
/**
 * Writes the PFM data using the indicated PNMWriter.  The PNMWriter is always
 * deleted upon completion, whether successful or not.
 */
bool PfmFile::
 ...
 * for each pixel.  Each valid point gets a 1 value; each nonexistent point gets
 * a 0 value.
 */
bool PfmFile::
 ...
 * Stores 1 or 0 values into the indicated PNMImage, according to has_point()
 * for each pixel.  Each valid point gets a 1 value; each nonexistent point gets
 * a 0 value.  This flavor of store_mask also checks whether the valid points
 * are within the specified min/max range.  Any valid points without the
 * condition min_point[c] <= value[c] <= max_point[c], for any c, are stored
...
/**
 * Fills the indicated channel with NaN, but only where the table already has a
 * data point.  Leaves empty points unchanged.
 */
void PfmFile::
 ...
 * assigned the value of their nearest neighbor.  Returns true if successful, or
 * false if the point value cannot be determined.
 */
bool PfmFile::
 ...
 * point index (x, y).  Returns true if the point has any contributors, false if
 ...
 * indicated vectors.  Returns true if successful, false if the mesh contains no
 * points.
 */
bool PfmFile::
 ...
 * points.  If there are no points with no_data_value in the grid--that is, all
 * points are included--then this will return (0, get_x_size(), 0,
 * get_y_size()).
 */
bool PfmFile::
...
/**
 * Returns true if all of the points on column x, from [y_begin, y_end), are the
 * no_data value, or false if any one of these points has a value.
 */
bool PfmFile::
...
/**
 * Sets the no_data_nan flag.  When num_channels is nonzero, then a NaN value in
 * any of the first num_channels channels indicates no data for that point.  If
 * num_channels is zero, then all points are valid.  This is a special case of
 * set_no_data_value().
 */
void PfmFile::
 ...
 * The dist map is understood to be a mapping of points in the range 0..1 in the
 * first two dimensions.  The operation can be expressed symbolically as:
 * this(u, v) = this(dist(u, v))  If scale_factor is not 1, it should be a value
 * > 1, and it specifies the factor to upscale the working table while
 * processing, to reduce artifacts from integer truncation.  By convention, the
 * y axis is inverted in the distortion map relative to the coordinates here.  A
 * y value of 0 in the distortion map corresponds with a v value of 1 in this
 * file.
 */
void PfmFile::
 ...
 * The dist map is understood to be a mapping of points in the range 0..1 in the
 * first two dimensions.  The operation can be expressed symbolically as:
 * this(u, v) = dist(this(u, v))  If scale_factor is not 1, it should be a value
 * > 1, and it specifies the factor to upscale the working table while
 * processing, to reduce artifacts from integer truncation.  By convention, the
 * y axis in inverted in the distortion map relative to the coordinates here.  A
 * y value of 0 in the distortion map corresponds with a v value of 1 in this
 * file.
 */
void PfmFile::
...
/**
 * Wherever there is missing data in this PfmFile (that is, wherever has_point()
 * returns false), copy data from the other PfmFile, which must be exactly the
 ...
 * corresponding point in this PfmFile to missing as well, so that this PfmFile
 * has only points where both files have points.  The point is set to "missing"
 * by setting it the no_data_value.
 */
void PfmFile::
...
/**
 * Copies just the specified channel values from the indicated PfmFile, but only
 * where the other file has a data point.
 */
void PfmFile::
...
/**
 * Applies delta * t to the point values within radius (xr, yr) distance of (xc,
 * yc).  The t value is scaled from 1.0 at the center to 0.0 at radius (xr, yr),
 * and this scale follows the specified exponent.  Returns the number of points
 * affected.
 */
int PfmFile::
 ...
 * Assumes the table contains 3-D points.  The return value is true if any
 * points in the table, or false if none are.
 */
bool PfmFile::
...
/**
 * Computes the minmax bounding volume of the points in 3-D space, assuming the
 * points represent a mostly-planar surface.  This algorithm works by sampling
 * the (square) sample_radius pixels at the four point_dist corners around the
 * center (cx - pd, cx + pd) and so on, to approximate the plane of the surface.
 * Then all of the points are projected into that plane and the bounding volume
 * of the entire mesh within that plane is determined.  If points_only is true,
 * the bounding volume of only those four points is determined.  center,
 * point_dist and sample_radius are in UV space, i.e.  in the range 0..1.
 */
 ...
 * distance) and the indicated point.  The point coordinates are given in UV
 ...
 * if the size is omitted, it defaults to the entire other image, or the largest
 * piece that will fit.
 */
void PfmFile::
...
/**
 * Behaves like copy_sub_image(), except the copy pixels are added to the pixels
 * of the destination, after scaling by the specified pixel_scale.
 */
void PfmFile::
...
/**
 * The implementation of has_point() for 1-component files with a no_data_value.
 */
bool PfmFile::
...
/**
 * The implementation of has_point() for 2-component files with a no_data_value.
 */
bool PfmFile::
...
/**
 * The implementation of has_point() for 3-component files with a no_data_value.
 */
bool PfmFile::
...
/**
 * The implementation of has_point() for 4-component files with a no_data_value.
 */
bool PfmFile::
...
/**
 * The implementation of has_point() for files with set_no_data_nan() in effect.
 * This means that the data is valid iff no components involve NaN.
 */
bool PfmFile::
