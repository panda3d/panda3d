...
output(ostream &out) const {
  ...
  check_rebuild();
  int geom_count = 0;
  if (_internal_geom != (PandaNode *)nullptr) {
...
apply_attribs_to_vertices(const AccumulatedAttribs &attribs, int attrib_types,
  ...
  if ((attrib_types & SceneGraphReducer::TT_color) != 0) {
    if (attribs._color != (const RenderAttrib *)nullptr) {
  ...
  if ((attrib_types & SceneGraphReducer::TT_color_scale) != 0) {
    if (attribs._color_scale != (const RenderAttrib *)nullptr) {
  ...
  if ((_flags & F_needs_rebuild) == 0 &&
      _internal_geom != (PandaNode *)nullptr) {
    SceneGraphReducer gr;
