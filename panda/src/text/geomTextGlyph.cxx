  ...
  Geom(data)
{
  ...
  if (glyph != (const TextGlyph *)nullptr) {
    _glyphs.reserve(1);
...
output(ostream &out) const {
  ...
  for (gi = _glyphs.begin(); gi != _glyphs.end(); ++gi) {
    ...
    nassertv(glyph != (const TextGlyph *)nullptr);
...
make_GeomTextGlyph(const FactoryParams &params) {
  GeomTextGlyph *me = new GeomTextGlyph((const TextGlyph *)nullptr,
                                        (GeomVertexData *)nullptr);
