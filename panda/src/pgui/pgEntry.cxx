  ...
  _obscure_text(get_text_node())
{
  ...
  _last_text_def = (TextNode *)nullptr;
...
set_text_def(int state, TextNode *node) {
  ...
  if (node == (TextNode *)nullptr && state >= (int)_text_defs.size()) {
...
get_text_def(int state) const {
  ...
  if (_text_defs[state] == (TextNode *)nullptr) {
    return get_text_node();
  }
...
slot_text_def(int state) {
  while (state >= (int)_text_defs.size()) {
    _text_defs.push_back((TextNode *)nullptr);
  }
}
...
update_text() {
  ...
  nassertv(node != (TextNode *)nullptr);
