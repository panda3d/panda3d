...
/**
 * Called after the node's draw_mask has been changed for any reason, this just
 * provides a hook so derived classes can do something special in this case.
 */
void PGItem::
draw_mask_changed() {
 ...
 * traversal from PGTop.  The return value is true if the region is valid, or
 ...
 * The mouse is only considered to be "entered" in one region at a time; in the
 * case of nested regions, it exits the outer region before entering the inner
 * one.
 */
void PGItem::
...
/**
 * This is a callback hook function, called whenever the mouse moves within the
 * boundaries of the region, even if it is also within the boundaries of a
 * nested region.  This is different from "enter", which is only called whenever
 * the mouse is within only that region.
 */
void PGItem::
...
/**
 * This is a callback hook function, called whenever the mouse moves completely
 * outside the boundaries of the region.  See within().
 */
void PGItem::
...
/**
 * Sets whether the PGItem currently has keyboard focus.  This simply means that
 * the item may respond to keyboard events as well as to mouse events; precisely
 * what this means is up to the individual item.  Only one PGItem in the world
 * is allowed to have focus at any given time.  Setting the focus on any other
 * item automatically disables the focus from the previous item.
 */
void PGItem::
 ...
 * enabled, the item will receive keypress events even if it is not in focus; in
 * fact, even if it is not onscreen.  Unlike normal focus, many items may have
 * background_focus simultaneously.
 */
void PGItem::
 ...
 * to the PGItem.  The complete set of state defs assigned may then be retrieved
 * by indexing from 0 to (get_num_state_defs() - 1).  This is only an upper
 * limit on the actual number of state defs, since there may be holes in the
 * list.
 */
int PGItem::
...
/**
 * Returns true if get_state_def() has ever been called for the indicated state
 * (thus defining a render subgraph for this state index), false otherwise.
 */
bool PGItem::
...
/**
 * Returns the Node that is the root of the subgraph that will be drawn when the
 * PGItem is in the indicated state.  The first time this is called for a
...
/**
 * Returns the sound associated with the indicated event, or NULL if there is no
 * associated sound.
 */
AudioSound *PGItem::
 ...
 * that it represents the largest part of the rectangular region passed in, that
 * does not overlap with the rectangular region of the indicated obscurer.  If
 * the obscurer is NULL, or is a hidden node, it is not considered and the frame
 * is left unchanged.  This is used by slider bars and scroll frames, which have
 * to automatically figure out how much space they have to work with after
 * allowing space for scroll bars and buttons.
 */
void PGItem::
...
/**
 * Returns the LVecBase4 frame of the indicated item, converted into this item's
 * coordinate space.  Presumably, item is a child of this node.
 */
LVecBase4 PGItem::
 ...
 * and modifies the points to reflect the new set of clipped points.  The return
 * value is true if the set of points is unmodified (all points are behind the
 * clip plane), or false otherwise.
 */
bool PGItem::
