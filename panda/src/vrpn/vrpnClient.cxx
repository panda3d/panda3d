...
/**
 * Writes a list of the active devices that the VrpnClient is currently polling
 * each frame.
 */
void VrpnClient::
...
/**
 * Creates and returns a new ClientDevice of the appropriate type, according to
 * the requested device_type and device_name.  Returns NULL if a matching device
 * cannot be found.  This is guaranteed not to be called twice for a given
 ...
 * data_type information.  The device_name may be one of the following:
 * tracker_name tracker_name:N tracker_name:N[pva]  Where N is an integer sensor
 * number, and [pva] is one of the lowercase letters p, v, or a.  In the first
 * form, the device connects to the indicated tracker, and reports position
 * information on sensor number 0.  In the second form, the device connects to
 * the indicated tracker, and reports position information on the indicated
 * sensor number.  In the third form, the device connects to the indicated
 * tracker, and reports either position, velocity, or acceleration information
...
/**
 * Removes and deletes the indicated VrpnTracker, which is no longer referenced
 * by any VrpnTrackerDevices.
 */
void VrpnClient::
...
/**
 * Finds a VrpnDial of the indicated name, and returns it if one already exists,
...
/**
 * Removes and deletes the indicated VrpnDial, which is no longer referenced by
 * any VrpnDialDevices.
 */
void VrpnClient::
 ...
 * case, this may need to come up with some kind of cacheing scheme so we don't
 * call mainloop multiple times when a user is just asking for the data of
 * multiple sensors on 1 tracker (as that is the interface supported).  This is
 * a non-trivial problem as it is difficult to know when we should and shouldn't
 * cache.
 */
void VrpnClient::
