...
/**
 * Reads all of the vfs-mount-url lines in the Config.prc file and replaces the
 * mount settings to match them.  This will mount any url's mentioned in the
 * config file, and unmount and unmount any url's no longer mentioned in the
 * config file.  Normally, it is called automatically at startup, and need not
 * be called again, unless you have fiddled with some config settings.
 */
...
/**
 * Returns an HTTPChannel object suitable for use for extracting a document from
 ...
 * standby duty, so that it will be returned by a future call to get_channel().
 */
void VirtualFileMountHTTP::
