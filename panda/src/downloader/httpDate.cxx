 ...
 * returns empty string.  A token is defined as a contiguous sequence of digits
 * or letters.  If it is a sequence of letters, the function quietly truncates
 * it to three letters before returning, and forces the first letter to capital
 * and the second two to lowercase.  If it is a sequence of digits, the function
 * also returns the next character following the last digit (unless it is a
 * letter).
 */
string HTTPDate::
