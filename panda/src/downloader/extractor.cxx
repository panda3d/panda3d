...
/**
 * Interrupts the Extractor in the middle of its business and makes it ready to
 * accept a new list of subfiles to extract.
 */
void Extractor::
reset() {
  if (_initiated) {
 ...
 * EU_ok.  step() extracts the next small unit of data from the Multifile.
 * Returns EU_ok if progress is continuing, EU_error_abort if there is a
 * problem, or EU_success when the last piece has been extracted.  Also see
 * run().
 */
int Extractor::
step() {
 ...
 * request_all_files() to specify the files you would like to extract, then call
 * run() to do the extraction.  Also see step() for when you would like the
 * extraction to happen as a background task.
 */
bool Extractor::
run() {
  while (true) {
