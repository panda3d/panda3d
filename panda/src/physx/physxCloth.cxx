...
/**
 * Returns true if this cloth is sleeping.  When a cloth does not move for a
...
/**
 * Wakes up the cloth if it is sleeping.  The wakeCounterValue determines how
 * long until the body is put to sleep, a value of zero means that the body is
 * sleeping.  wake_up(0) is equivalent to PhysxCloth::put_to_sleep().
 */
void PhysxCloth::
...
/**
 * Forces the cloth to sleep.  The cloth  will stay asleep until the next call
 * to simulate, and will not wake up until then even when otherwise it would
 * (for example a force is applied to it). It can however wake up during the
 * next do_physics call.
 */
void PhysxCloth::
put_to_sleep() {
...
/**
 * Sets the linear velocity below which an cloth may go to sleep.  Cloths whose
...
/**
 * Returns the linear velocity below which an soft body may go to sleep.  cloths
