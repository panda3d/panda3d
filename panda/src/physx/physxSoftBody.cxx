...
/**
 * Gets the relative grid spacing for the broad phase.  The cloth is represented
 * by a set of world aligned cubical cells in broad phase.  The size of these
 * cells is determined by multiplying the length of the diagonal of the AABB of
 * the initial soft body size with this constant.
 */
float PhysxSoftBody::
...
/**
 * Returns true if this soft body is sleeping.  When a soft body does not move
 * for a period of time, it is no longer simulated in order to save time.  This
 * state is called sleeping.  However, because the object automatically wakes up
 * when it is either touched by an awake object, or one of its properties is
 * changed by the user, the entire sleep mechanism should be transparent to the
 * user.
 */
bool PhysxSoftBody::
is_sleeping() const {
...
/**
 * Wakes up the soft body if it is sleeping.  The wakeCounterValue determines
 * how long until the body is put to sleep, a value of zero means that the body
 * is sleeping.  wake_up(0) is equivalent to PhysxSoftBody::put_to_sleep().
 */
void PhysxSoftBody::
...
/**
 * Forces the soft body to sleep.  The soft body  will stay asleep until the
 * next call to simulate, and will not wake up until then even when otherwise it
 * would (for example a force is applied to it). It can however wake up during
...
/**
 * Sets the linear velocity below which an soft body may go to sleep.  SoftBodys
 ...
 * are attached.  This method only works with primitive and convex shapes.
 * Since the inside of a general triangle mesh is not clearly defined.
 */
void PhysxSoftBody::
...
/**
 * Attaches the cloth to all shapes, currently colliding.  This method only
 * works with primitive and convex shapes.  Since the inside of a general
...
/**
 * Detaches the cloth from a shape it has been attached to before.  If the cloth
 * has not been attached to the shape before, the call has no effect.
 */
void PhysxSoftBody::
...
/**
 * Returns the pointer to an attached shape pointer of the given vertex.  If the
 * vertex is not attached or attached to a global position, NULL is returned.
 */
...
/**
 * Sets an external acceleration which affects all non attached particles of the
 * cloth.
 */
void PhysxSoftBody::
...
/**
 * Retrieves the external acceleration which affects all non attached particles
 * of the cloth.
 */
