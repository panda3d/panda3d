 ...
 * won't take part in collisions, but trigger events if some other shape passes
 * through it.
 */
void PhysxShapeDesc::
...
/**
 * Specifies by how much shapes can interpenetrate.  Two shapes will
 * interpenetrate by the sum of their skin widths.  This means that their
 * graphical representations should be adjusted so that they just touch when the
 * shapes are interpenetrating.  The default skin width is the 'physx-skin-
 * width' parameter.  A skin width sum of zero for two bodies is not permitted
 * because it will lead to an unstable simulation.  If your simulation jitters
 * because resting bodies occasionally lose contact, increasing the size of your
 * collision volumes and the skin width may improve things.
 */
void PhysxShapeDesc::
 ...
 * properties for a rigidbody.  When mass<=0.0 then density and volume determine
 * the mass.  Note that this will only be used if the body has a zero inertia
 * tensor, or if you call PhysxActor::update_mass_from_shapes explicitly.
 */
void PhysxShapeDesc::
set_mass(float mass) {
 ...
 * properties for a rigidbody (unless a valid mass >0.0 is provided). Note that
