 ...
 * directed toward the ground.  A ray is cast from the shape's origin along the
 * -Y axis.  When the ray strikes something, and the distance is:  - less than
 * wheelRadius from the shape origin: a hard contact is created - between
 * wheelRadius and (suspensionTravel + wheelRadius): a soft suspension contact
 * is created - greater than (suspensionTravel + wheelRadius): no contact is
 * created.  Thus at the point of greatest possible suspension compression the
 * wheel axle will pass through at the shape's origin.  At the point greatest
 * suspension extension the wheel axle will be a distance of suspensionTravel
 * from the shape's origin.  The suspension's targetValue is 0 for real cars,
 * which means that the suspension tries to extend all the way.  Otherwise one
 * can specify values [0,1] for suspensions which have a spring to pull the
 * wheel up when it is extended too far.  0.5 will then fall halfway along
 * suspensionTravel.  The +Z axis is the 'forward' direction of travel for the
 * wheel.  -Z is backwards.  The wheel rolls forward when rotating around the
 * positive direction around the X axis.  A positive wheel steering angle
 * corresponds to a positive rotation around the shape's Y axis.  (Castor angles
 * are not modeled.)  The coordinate frame of the shape is rigidly fixed on the
 * car.
 */
