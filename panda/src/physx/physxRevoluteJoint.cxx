...
/**
 * Retrieves the current revolute joint angle.  The relative orientation of the
 * bodies is stored when the joint is created, or when set_axis() or
 * set_anchor() is called.  This initial orientation returns an angle of zero,
 * and joint angles are measured relative to this pose.  The angle is in the
 * range [-180, 180], with positive angles CCW around the axis, measured from
 * body2 to body1.
 */
...
/**
 * Retrieves the revolute joint angle's rate of change (angular velocity). It is
 * the angular velocity of body1 minus body2 projected along the axis.
 */
float PhysxRevoluteJoint::
get_velocity() const {
...
/**
 * Sets spring parameters.  The spring is implicitly integrated so no
 * instability should result for arbitrary spring and damping constants.  Using
 * these settings together with a motor is not possible -- the motor will have
 * priority and the spring settings are ignored.  If you would like to simulate
 * your motor's internal friction, do this by altering the motor parameters
 * directly.  spring - The rotational spring acts along the hinge axis and tries
 * to force the joint angle to zero.  A setting of zero disables the spring.
 * Default is 0, should be >= 0.  damper - Damping coefficient; acts against the
 * hinge's angular velocity.  A setting of zero disables the damping.  The
 * default is 0, should be >= 0.  targetValue - The angle at which the spring is
 * relaxed.  In [-Pi,Pi]. Default is 0.  This automatically enables the spring
 */
void PhysxRevoluteJoint::
...
/**
 * Sets angular joint limits.  If either of these limits are set, any planar
 * limits in PhysxJoint are ignored.  The limits are angles defined the same way
 * as the values that get_angle() returns.  The following has to hold:  Pi <
 * lowAngle < highAngle < Pi Both limits are disabled by default.  Also sets
 * coefficients of restitutions for the low and high angular limits.  These
 * settings are only used if valid limits are set using set_limits(). These
 * restitution coefficients work the same way as for contacts.  The coefficient
 * of restitution determines whether a collision with the joint limit is
 * completely elastic (like pool balls, restitution = 1, no energy is lost in
 * the collision), completely inelastic (like putty, restitution = 0, no rebound
 * after collision) or somewhere in between.  The default is 0 for both.  This
 * automatically enables the limit.
 */
void PhysxRevoluteJoint::
