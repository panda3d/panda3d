...
/**
 * Set the max scene bounds.  If scene bounds are provided (maxBounds in the
 * descriptor), the SDK takes advantage of this information to accelerate scene-
 * level collision queries (e.g.  raycasting). When using maxBounds, you have to
 * make sure created objects stay within the scene bounds.  In particular, the
 * position of dynamic shapes should stay within the provided bounds.  Otherwise
 * the shapes outside the bounds will not be taken into account by all scene
 * queries (raycasting, sweep tests, overlap tests, etc). They will nonetheless
 * still work correctly for the main physics simulation.
 */
void PhysxSceneDesc::
...
/**
 * Defines the number of broadphase cells along the grid x-axis.  Must be power
 * of two.  Max is 8 at the moment.  The broadphase type must be set to
...
/**
 * Defines the number of broadphase cells along the grid y-axis.  Must be power
 * of two.  Max is 8 at the moment.  The broadphase type must be set to
 * BPT_sap_multi for this parameter to have an effect.
 */
void PhysxSceneDesc::
...
/**
 * Defines which type of broadphase to use.  (1) BPT_sap_single: A sweep-and-
 * prune (SAP) algorithm to find pairs of potentially colliding shapes.  (2)
 * BPT_sap_multi: A multi sweep-and-prune algorithm to find pairs of potentially
 * colliding shapes.  Uses a configurable 2D grid to divide the scene space into
 * cells.  The potentially overlapping shape pairs are detected in each cell and
 * the information is merged together.  This approach is usually faster than
 * BPT_sap_single in scenarios with many shapes and a high creation/deletion
 * rate of shapes.  However, the amount of memory required is considerably
 * higher depending on the number of grid cells used.  The following extra
 * parameters need to be defined: - PhysxSceneDesc.set_max_bounds -
 * PhysxSceneDesc.set_num_grid_cells_x - PhysxSceneDesc.set_num_grid_cells_y
