  ...
  : InteractiveGraphicsPipe(spec)
{
  ...
  if (dx_full_screen)
	  {
	  WNDCLASS wndClass = { CS_HREDRAW | CS_VREDRAW, (WNDPROC)static_window_proc, 0, 0, NULL,
                          NULL, NULL, (HBRUSH)GetStockObject(WHITE_BRUSH), NULL,
                          TEXT("Fullscreen Window") };

	  if (!RegisterClass( &wndClass ))  {
		    wdxdisplay_cat.fatal()
		      << "wdxGraphicsPipe::construct(): could not register full screen window "
			  << "class" << endl;
		    exit(0);
		  }
	  }
  ...
  if (!RegisterClass(&wc)) {
    wdxdisplay_cat.fatal()
      << "wdxGraphicsPipe::construct(): could not register window "
	<< "class" << endl;
    exit(0);
  }
...
//  Description:
////////////////////////////////////////////////////////////////////
LONG wdxGraphicsPipe::
window_proc(HWND hwnd, UINT msg, WPARAM wparam, LPARAM lparam) {
  PAINTSTRUCT ps;
  wdxGraphicsWindow *window;
  int button = -1;
...
//		  window->make_current();
    ...
    case WM_MOVE:
      window = find_window(hwnd);
      if (window) {
		window->handle_window_move(LOWORD(lparam), HIWORD(lparam) );
		}
      return 0;
...
////////////////////////////////////////////////////////////////////
ButtonHandle
wdxGraphicsPipe::lookup_key(WPARAM wparam) const {
  switch (wparam) {
