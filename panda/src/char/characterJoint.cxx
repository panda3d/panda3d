...
/**
 * This is called by do_update() whenever the part or some ancestor has changed
 * values.  It is a hook for derived classes to update whatever cache they may
 * have that depends on these.  The return value is true if the part has changed
 * as a result of the update, or false otherwise.  In the case of a
 * CharacterJoint, of course, it means to recompute the joint angles and
 * associated transforms for this particular joint.
 */
bool CharacterJoint::
...
/**
 * Adds the indicated node to the list of nodes that will be updated each frame
 * with the joint's net transform from the root.  Returns true if the node is
 * successfully added, false if it had already been added.  A
 * CharacterJointEffect for this joint's Character will automatically be added
 ...
 * frame with the joint's net transform from the root.  Returns true if the node
 * is successfully removed, false if it was not on the list.  If the node has a
 * CharacterJointEffect that matches this joint's Character, it will be cleared.
 */
bool CharacterJoint::
...
/**
 * Removes all nodes from the list of nodes that will be updated each frame with
 * the joint's net transform from the root.
 */
void CharacterJoint::
...
/**
 * Adds the indicated node to the list of nodes that will be updated each frame
 * with the joint's local transform from its parent.  Returns true if the node
 * is successfully added, false if it had already been added.  The Character
 * pointer should be the Character object that owns this joint; this will be
 * used to create a CharacterJointEffect for this node.  If it is NULL, no such
 * effect will be created.  A CharacterJointEffect for this joint's Character
...
/**
 * Removes the indicated node from the list of nodes that will be updated each
 * frame with the joint's local transform from its parent.  Returns true if the
 * node is successfully removed, false if it was not on the list.  If the node
 * has a CharacterJointEffect that matches this joint's Character, it will be
 * cleared.
 */
bool CharacterJoint::
...
/**
 * Removes all nodes from the list of nodes that will be updated each frame with
 * the joint's local transform from its parent.
 */
void CharacterJoint::
 ...
 * returns a list of NodePaths, even though the local transforms are actually a
 * list of PandaNodes.
 */
NodePathCollection CharacterJoint::
