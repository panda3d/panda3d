 ...
 * creates a new one if the munger has not yet been used to render this context.
 * Fills index with the display list index, and returns true if the display list
 * is current (that is, it has the same modified stamp).
 */
...
/**
 * Called only from the draw thread by GLGraphicsStateGuardian::release_geom(),
 * this should delete all of the queued display lists immediately.
 */
