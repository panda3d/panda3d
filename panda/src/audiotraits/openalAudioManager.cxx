...
/**
 * This makes this manager's OpenAL context the current context.  Needed before
 * any parameter sets.
 */
void OpenALAudioManager::
make_current() const {
...
/**
 * Returns true if the specified MovieAudio should be cached into RAM.  A lot of
 * conditions have to be met in order to allow caching - if any are not met, the
...
/**
 * Obtains a SoundData for the specified sound.  When you are done with the
 * SoundData, you need to decrement the client count.
 */
 ...
 * IMPORTANT! Both Panda3D and OpenAL use a right handed coordinate system.  But
 * there is a major difference!  In Panda3D the Y-Axis is going into the Screen
 * and the Z-Axis is going up.  In OpenAL the Y-Axis is going up and the Z-Axis
 * is coming out of the screen.  The solution is simple, we just flip the Y and
 * Z axis and negate the Z, as we move coordinates from Panda to OpenAL and
 * back.  What does did mean to average Panda user?  Nothing, they shouldn't
 * notice anyway.  But if you decide to do any 3D audio work in here you have to
 * keep it in mind.  I told you, so you can't say I didn't.
 */
void OpenALAudioManager::
...
/**
 * Set units per foot WARNING: OpenAL has no distance factor but we use this as
 * a scale on the min/max distances of sounds to preserve FMOD compatibility.
 * Also, adjusts the speed of sound to compensate for unit difference.  OpenAL's
 * default speed of sound is 343.3 m/s == 1126.3 ft/s
 */
void OpenALAudioManager::
...
/**
 * Inform the manager that a sound is about to play.  The manager will add this
 * sound to the table of sounds that are playing, and will allocate a source to
 * this sound.
 */
void OpenALAudioManager::
...
/**
 * Discards sounds from the sound cache until the number of sounds remaining is
 * under the limit.
 */
void OpenALAudioManager::
