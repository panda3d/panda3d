...
munge_format_impl(const GeomVertexFormat *orig,
  ...
  if (vertex_type != (const GeomVertexColumn *)nullptr) {
  ...
  if (_filtered_texture != (TextureAttrib *)nullptr) {
    ...
    for (tc_index = 0; tc_index <= max_tc_index; ++tc_index) {
      ...
      if (texcoord_type != (const GeomVertexColumn *)nullptr) {
        new_array_format->add_column
