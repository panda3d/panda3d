...
/**
 * Changes the number of threads for this task chain.  This may require stopping
 * the threads if they are already running.
 */
void AsyncTaskChain::
...
/**
 * Sets the maximum amount of time per frame the tasks on this chain are granted
 * for execution.  If this is less than zero, there is no limit; if it is >= 0,
 * it represents a maximum amount of time (in seconds) that will be used to
 * execute tasks.  If this time is exceeded in any one frame, the task chain
 * will stop executing tasks until the next frame, as defined by the
 * TaskManager's clock.
 */
void AsyncTaskChain::
 ...
 * forced to sync with the TaskManager's clock.  It will run no faster than one
 * epoch per clock frame.  When this flag is false, the default, the task chain
 * will finish all of its tasks and then immediately start from the first task
 * again, regardless of the clock frame.  When it is true, the task chain will
 * finish all of its tasks and then wait for the clock to tick to the next frame
 * before resuming the first task.  This only makes sense for threaded task
 * chains.  Non-threaded task chains are automatically synchronous.
 */
void AsyncTaskChain::
 ...
 * priority, and the number of times per epoch each task will run.  When this
 * flag is true, some tasks might not run in any given epoch.  Instead, tasks
 * with priority higher than 1 will be given precedence, in proportion to the
 * amount of time they have already used.  This gives higher-priority tasks more
 * runtime than lower-priority tasks.  Each task gets the amount of time
 * proportional to its priority value, so a task with priority 100 will get five
 * times as much processing time as a task with priority 20.  For these
 * purposes, priority values less than 1 are deemed to be equal to 1.  When this
 * flag is false (the default), all tasks are run exactly once each epoch,
 * round-robin style.  Priority is only used to determine which task runs first
 * within tasks of the same sort value.
 */
void AsyncTaskChain::
...
/**
 * Returns the number of tasks that are currently active or sleeping within the
 * task chain.
 */
int AsyncTaskChain::
get_num_tasks() const {
 ...
 * already added to this or any other active queue.  This is normally called
 * only by the AsyncTaskManager.  Assumes the lock is already held.
 */
void AsyncTaskChain::
 ...
 * otherwise.  Assumes the lock is already held.  The task->upon_death() method
 * is *not* called.
 */
bool AsyncTaskChain::
 ...
 * called on destruction only.  Assumes the lock is already held.
 */
void AsyncTaskChain::
do_cleanup() {
  if (task_cat.is_spam()) {
...
/**
 * Pops a single task off the active queue, services it, and restores it to the
 * end of the queue.  This is called internally only within one of the task
 * threads.  Assumes the lock is already held.  Note that the lock may be
 * temporarily released by this method.
 */
void AsyncTaskChain::
 ...
 * about to be removed from the active queue.  Assumes the lock is held.  If
 * upon_death is true, then task->upon_death() will also be called, with the
 * indicated clean_exit parameter.  Note that the lock may be temporarily
 ...
 * and it is time to increment to the next sort value, or begin the next epoch.
 * Assumes the lock is held.  Returns true if there are more tasks on the queue
 * after this operation, or false if the task list is empty and we need to wait.
 */
bool AsyncTaskChain::
finish_sort_group() {
 ...
 * that have already exceeded their priority budget for this epoch.  Assumes the
 ...
 * _active and _next_active lists as they should have been without pickup mode,
 * for next frame.  Assumes the lock is held.
 */
void AsyncTaskChain::
cleanup_pickup_mode() {
  if (_pickup_mode) {
