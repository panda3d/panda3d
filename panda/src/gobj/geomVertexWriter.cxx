 ...
 * array.  This also resets the current write row number to the start row (the
 * same value passed to a previous call to set_row(), or 0 if set_row() was
 * never called.)  The return value is true if the data type is valid, false
 * otherwise.
 */
...
/**
 * This ensures that enough memory space for num_rows is allocated, so that you
 * may add up to num_rows rows without causing a new memory allocation.  This is
 * a performance optimization only; it is especially useful when you know the
 * number of rows you will be adding ahead of time.
 */
bool GeomVertexWriter::
