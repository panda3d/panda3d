 ...
 * per vertex.  The return value is the index number of the new data type.
 */
 ...
 * floating-point numbers such as "vertex" or "normal"; you must specify where
 * in each record the table starts, and how many components (dimensions) exist
 * per vertex.  Adding a column with the same name as a previous type, or that
 * overlaps with one or more previous types, quietly removes the previous
 * type(s).  The return value is the index number of the new data type.
 */
int GeomVertexArrayFormat::
...
/**
 * Removes the column with the indicated name, if any.  This leaves a gap in the
 * byte structure.
 */
void GeomVertexArrayFormat::
 ...
 * (assuming SSE2 is enabled in the build).  The caller is responsible for
 * testing vertex_animation_align_16 to decide whether to call this method.
 */
void GeomVertexArrayFormat::
...
/**
 * Returns the first specification that overlaps with any of the indicated bytes
 * in the range, or NULL if none do.
 */
const GeomVertexColumn *GeomVertexArrayFormat::
 ...
 * equivalent in the other array format, and in the same byte positions, and the
 * stride is the same.  That is, true if this format can share the same data
 * pointer as the other format (with possibly some unused gaps).
 */
bool GeomVertexArrayFormat::
...
/**
 * Resorts the _columns vector so that the columns are listed in the same order
 ...
 * GeomVertexArrayFormat is encountered in the Bam file.  It should create the
 * GeomVertexArrayFormat and extract its information from the file.
 */
