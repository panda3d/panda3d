...
/**
 * Changes the reserved size of the buffer, preserving its data (except for any
 * data beyond the new end of the buffer, if the buffer is being reduced).  If
 * the buffer is expanded, the new data is uninitialized.  Assumes the lock is
 * already held.
 */
void VertexDataBuffer::
 ...
 * data.  This implicitly resets the size to 0.  Assumes the lock is already
 * held.
 */
void VertexDataBuffer::
 ...
 * implicitly move it off of the page and back into independent memory.  Assumes
...
/**
 * Moves the buffer off of its current page and into independent memory.  If the
 * page is not already resident, it is forced resident first.  Assumes the lock
 * is already held.
 */
void VertexDataBuffer::
do_page_in() {
