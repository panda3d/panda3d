...
/**
 * Copying a vertex pool is of questionable value, since it will copy all of the
 * vertices and assign new pointers to them all.  There will be no polygons
...
/**
 * Returns true if any vertices in the pool are fully defined vertices, false if
 * all vertices are forward references.
 */
bool EggVertexPool::
...
/**
 * Returns the vertex in the pool with the indicated index number, or NULL if no
 * vertices have that index number.
 */
...
/**
 * Returns the vertex in the pool with the indicated index number.  If there is
 * not a vertex in the pool with the indicated index number, creates a special
 * forward-reference EggVertex that has no data, on the assumption that the
 * vertex pool has not yet been fully read and more data will be available
 * later.
 */
EggVertex *EggVertexPool::
...
/**
 * Returns true if any vertex in the pool has a normal defined, false if none of
 * them do.
 */
bool EggVertexPool::
has_normals() const {
...
/**
 * Returns true if any vertex in the pool has a color defined, false if none of
 * them do.
 */
bool EggVertexPool::
has_colors() const {
...
/**
 * Returns true if any vertex in the pool has a color defined other than white,
 * false if no vertices have colors, or if all colors are white.
 */
bool EggVertexPool::
...
/**
 * Returns the list of UV names that are defined by any vertices in the pool, as
 * well as the subset of UV names that actually define 3-d texture coordinates
 * ("uvw_names").  Also returns the subset of UV/UVW names that define a tangent
 * and binormal.  It is the user's responsibility to clear both vectors before
 * calling this method.
 */
void EggVertexPool::
...
/**
 * Returns the list of auxiliary data names that are defined by any vertices in
 * the pool.
 */
void EggVertexPool::
...
/**
 * Returns an iterator that can be used to traverse through all the vertices in
 * the pool.
 */
 ...
 * pool.  If the index number is supplied, tries to assign that index number; it
 * is an error if the index number is already in use.  It is possible that a
 * forward reference to this vertex was requested in the past; if so, the data
 * from the supplied vertex is copied onto the forward reference, which becomes
 * the actual vertex.  In this case, a different pointer is saved (and returned)
 * than the one actually passed in.  In the usual case, however, the vertex
 * pointer passed in is the one that is saved in the vertex pool and returned
 * from this method.
 */
EggVertex *EggVertexPool::
 ...
 * returns it.  If there is already a vertex in the pool like the indicated one,
 ...
 * vertices that are attached to primitives that believe their vertices are in a
 * local coordinate system are transformed only by the scale and rotation
