...
/**
 * Sets the user data associated with this object.  This may be any EggUserData-
 * derived object.  The egg library will do nothing with this pointer, except to
 * hold its reference count and return the pointer on request.  The EggObject
 * maintains multiple different EggUserData pointers, one for each unique type
 * (as reported by get_type()).  If you know that only one type of EggUserData
 * object will be added in your application, you may use the query functions
 * that accept no parameters, but it is recommended that in general you pass in
 * the type of your particular user data, to allow multiple applications to
 * coexist in the same egg data.  This pointer is also copied by the copy
...
/**
 * Returns the user data pointer most recently stored on this object, or NULL if
...
/**
 * Returns the user data pointer of the indicated type, if it exists, or NULL if
 * it does not.
 */
EggUserData *EggObject::
...
/**
 * Returns true if a generic user data pointer has recently been set and not yet
