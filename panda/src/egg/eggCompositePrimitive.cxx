...
/**
 * Subdivides the composite primitive into triangles and adds those triangles to
 * the parent group node in place of the original primitive.  Returns a pointer
 * to the original primitive, which is likely about to be destructed.  If
 * convex_also is true, both concave and convex polygons will be subdivided into
 * triangles; otherwise, only concave polygons will be subdivided, and convex
 * polygons will be copied unchanged into the container.
 */
 ...
 * triangles of this triangle strip.  It is assumed that the
 * EggCompositePrimitive is not already a child of any other group when this
 * function is called.  Returns true if the triangulation is successful, or
 * false if there was some error (in which case the container may contain some
