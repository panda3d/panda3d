...
/**
 * Adds a series of EggMaterial nodes to the beginning of the indicated node to
 * reflect each of the materials in the collection.  Returns an iterator
 ...
 * the collection, adding them to the collection.  If this is called following
 * extract_materials(), it can be used to pick up any additional material
 * references that appeared in the egg hierarchy (but whose EggMaterial node was
 * not actually part of the hierarchy).  If this is called in lieu of
 * extract_materials(), it will fill up the collection with all of the
 * referenced materials (and only the referenced materials), without
 * destructively removing the EggMaterials from the hierarchy.  This also has
 * the side effect of incrementing the internal usage count for a material in
 * the collection each time a material reference is encountered.  This side
 * effect is taken advantage of by remove_unused_materials().
 */
int EggMaterialCollection::
 ...
 * materials removed.  This flavor of collapse_equivalent_materials()
 * automatically adjusts all the primitives in the egg hierarchy to refer to the
 * new material pointers.
 */
int EggMaterialCollection::
 ...
 * that are equivalent according to the indicated equivalence factor, eq (see
 * EggMaterial::is_equivalent_to()).  The return value is the number of
 * materials removed.  This flavor of collapse_equivalent_materials() does not
 * adjust any primitives in the egg hierarchy; instead, it fills up the
 * 'removed' map with an entry for each removed material, mapping it back to the
 * equivalent retained material.  It's up to the user to then call
 * replace_materials() with this map, if desired, to apply these changes to the
 * egg hierarchy.
 */
int EggMaterialCollection::
 ...
 * collapse_equivalent_materials().  It does not directly affect the Collection.
 */
void EggMaterialCollection::
...
/**
 * Guarantees that each material in the collection has a unique MRef name.  This
 ...
 * material was added, false if it was already there or if there was some error.
 */
