...
/**
 * Runs the animation from the frame "from" to and including the frame "to", at
 * which point the animation is stopped.  Both "from" and "to" frame numbers may
 * be outside the range (0, get_num_frames()) and the animation will follow the
 * range correctly, reporting numbers modulo get_num_frames().  For instance,
 * play(0, get_num_frames() * 2) will play the animation twice and then stop.
 */
void AnimInterface::CData::
 ...
 * Unlike the value returned by get_frame(), this frame number may extend beyond
 * the range of get_num_frames() if the frame range passed to play(), loop(),
 * etc.  did.  Unlike the value returned by get_full_fframe(), this return value
 * will never exceed the value passed to to_frame in the play() method.
 */
...
/**
 * Returns the current floating-point frame number.  Unlike the value returned
 * by get_frame(), this frame number may extend beyond the range of
 * get_num_frames() if the frame range passed to play(), loop(), etc.  did.
 * Unlike the value returned by get_full_frame(), this return value may equal
 * (to_frame + 1.0), when the animation has played to its natural end.  However,
 * in this case the return value of get_full_frame() will be to_frame, not
 * (to_frame + 1).
 */
 ...
 * (e.g.  because stop() or pose() was called, or because it reached the end of
 * the animation after play() was called).
 */
...
/**
 * Returns the current floating-point frame number, elapsed since _start_frame.
 */
double AnimInterface::CData::
get_f() const {
  if (_paused) {
