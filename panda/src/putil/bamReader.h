 ...
 * object's read pass.  To use this, subclass BamReaderAuxData and add whatever
 ...
 * file, as generated by a BamWriter.  A Bam file can be thought of as a linear
 * collection of objects.  Each object is an instance of a class that inherits,
 * directly or indirectly, from TypedWritable.  The objects may include pointers
 * to other objects within the Bam file; the BamReader automatically manages
 * these (with help from code within each class) and restores the pointers
 * correctly.  This is the abstract interface and does not specifically deal
 * with disk files, but rather with a DatagramGenerator of some kind, which is
 * simply a linear source of Datagrams.  It is probably from a disk file, but it
 * might conceivably be streamed directly from a network or some such nonsense.
