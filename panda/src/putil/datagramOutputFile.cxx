...
/**
 * Opens the indicated filename for writing.  Returns true if successful, false
 * on failure.
 */
...
/**
 * Copies the file data from the entire indicated file (via the vfs) as the next
 * datagram.  This is intended to support potentially very large datagrams.
 * Returns true on success, false on failure or if this method is unimplemented.
 * On true, fills "result" with the information that references the copied file,
 * if possible.
 */
bool DatagramOutputFile::
 ...
 * large datagrams.  Returns true on success, false on failure or if this method
 * is unimplemented.  On true, fills "result" with the information that
...
/**
 * Returns the filename that provides the target for these datagrams, if any, or
 * empty string if the datagrams do not get written to a file on disk.
 */
...
/**
 * Returns the current file position within the data stream, if any, or 0 if the
 * file position is not meaningful or cannot be determined.  For
 * DatagramOutputFiles that return a meaningful file position, this will be
