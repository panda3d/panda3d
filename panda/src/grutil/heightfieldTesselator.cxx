...
/**
 * Makes sure that the heightfield is a grayscale image of valid dimensions.  If
 * necessary, adds a band of zeros onto two sides of the heightfield, so as to
 * make the size of the heightfield a multiple of the given size plus one.
 */
 ...
 * preconditions:  1. A square of scale N can be adjacent to a square of scale N
 * or scale N-1, but not scale N-2 or smaller.  2. A square of scale N can be
 * adjacent to at most one square of scale N-1.  Precondition 1 is assured by
 * spacing out the detail radii sufficiently.  Precondition 2 is assured by
 * using rectangular detail radii.  I may someday rewrite this code to eliminate
...
/**
 * Adds a quad to the current geom.  Eventually, I plan to reimplement this.  It
 * is going to add a quad to a table of quads.  A post-processing pass is going
 * to traverse the table, calling add_quad_to_strip in the optimal order.  For
 * now, though, this routine just calls add_quad_to_strip directly, which is
 * quite inefficient.
 */
void HeightfieldTesselator::
