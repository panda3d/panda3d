 ...
 * relationship that is used to construct the scene graph, with the same sort of
 * nodes and NodePaths.  In a data graph, each node may potentially produce
 * andor consume data, and the arcs transmit data downward, from the root of the
 * graph to its leaves.  Thus, an input device such as a mouse might be added to
 * the graph near the root, and a tformer-style object that interprets the mouse
 * data as a trackball motion and outputs a matrix might be the immediate child
 * of the mouse, followed by an object that accepts a matrix and sets it on some
 * particular arc in the scene graph.  Each different kind of DataNode defines
 * its own set of input values and output values, identified by name.  When a
 * DataNode is attached to another DataNode, the inputs of the child are
 * automatically connected up to the corresponding outputs of the parent, and an
 ...
 * inputs and outputs in the constructor.  DataNode does not attempt to cycle
 * its data with a PipelineCycler.  The data graph is intended to be used only
