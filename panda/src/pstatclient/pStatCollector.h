...
/**
 * A lightweight class that represents a single element that may be timed and/or
 * counted via stats.  Collectors can be used to measure two different kinds of
 * values: elapsed time, and "other".  To measure elapsed time, call start() and
 * stop() as appropriate to bracket the section of code you want to time (or use
 * a PStatTimer to do this automatically).  To measure anything else, call
 * set_level() and/or add_level() to set the "level" value associated with this
 * collector.  The meaning of the value set for the "level" is entirely up to
 * the user; it may represent the number of triangles rendered or the kilobytes
 * of texture memory consumed, for instance.  The level set will remain fixed
 * across multiple frames until it is reset via another set_level() or adjusted
 * via a call to add_level().  It may also be completely removed via
 * clear_level().
 */
