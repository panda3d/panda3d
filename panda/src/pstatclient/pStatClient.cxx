 ...
 * concatenation of all of the collector's parents' names (except Frame) and the
 ...
 * PStatCollectors will be counted in if they do not specify otherwise.
 */
PStatThread PStatClient::
...
/**
 * A convenience function to call new_frame() on any threads with the indicated
 * sync_name
 */
void PStatClient::
...
/**
 * Returns a PStatCollector suitable for measuring categories with the indicated
 * name.  This is normally called by a PStatCollector constructor.  The name may
 * contain colons; if it does, it specifies a relative path to the client
 * indicated by the parent index.
 */
PStatCollector PStatClient::
...
/**
 * Returns a PStatCollector suitable for measuring categories with the indicated
 * name.  This is normally called by a PStatCollector constructor.  The name
...
/**
 * Returns true if the indicated collector/thread combination is active, and we
 * are transmitting stats data, or false otherwise.  Normally you would not use
 * this interface directly; instead, call PStatCollector::is_active().
 */
bool PStatClient::
...
/**
 * Returns true if the indicated collector/thread combination has been started,
 * or false otherwise.  Normally you would not use this interface directly;
...
/**
 * Removes the level value from the indicated collector.  The collector will no
 * longer be reported as having any particular level value.  Normally you would
 * not use this interface directly; instead, call PStatCollector::clear_level().
 */
void PStatClient::
 ...
 * given collector.  If the collector does not already have a level value, it is
 * initialized to 0.  Normally you would not use this interface directly;
 ...
 * pstats client is not connected.  Normally you would not use this interface
 * directly; instead, call PStatCollector::get_level().
 */
double PStatClient::
...
/**
 * Adds a new Collector entry to the _collectors array, in a thread-safe manner.
 * Assumes _lock is already held.
 */
void PStatClient::
