...
/**
 * Normally, a PartBundle constructor should not be called directly--it will get
 * created when a PartBundleNode is created.
 */
PartBundle::
 ...
 * animation and set it on the current one.  However, if this flag is set true,
 * the control_effect must be explicitly set via set_control_effect() whenever
 * an animation is to affect the character.
 */
void PartBundle::
...
/**
 * Returns a PartBundle that is a duplicate of this one, but with the indicated
 * transform applied.  If this is called multiple times with the same
 * TransformState pointer, it returns the same PartBundle each time.
 */
...
/**
 * Sets the control effect of all AnimControls to zero (but does not "stop" the
 * AnimControls).  The character will no longer be affected by any animation,
 * and will return to its default pose (unless restore-initial-pose is false).
...
/**
 * Binds the animation to the bundle, if possible, and returns a new AnimControl
 * that can be used to start and stop the animation.  If the anim hierarchy does
 * not match the part hierarchy, returns NULL.  If hierarchy_match_flags is 0,
 * only an exact match is accepted; otherwise, it may contain a union of
 * PartGroup::HierarchyMatchFlags values indicating conditions that will be
 * tolerated (but warnings will still be issued).  If subset is specified, it
 * restricts the binding only to the named subtree of joints.  The AnimControl
 * is not stored within the PartBundle; it is the user's responsibility to
 * maintain the pointer.  The animation will automatically unbind itself when
 * the AnimControl destructs (i.e.  its reference count goes to zero).
 */
...
/**
 * Binds an animation to the bundle.  The animation is loaded from the disk via
 * the indicated Loader object.  In other respects, this behaves similarly to
 * bind_anim(), with the addition of asynchronous support.  If allow_aysnc is
 * true, the load will be asynchronous if possible.  This requires that the
 * animation basename can be found in the PartBundle's preload table (see
 * get_anim_preload()).  In an asynchronous load, the animation file will be
 * loaded and bound in a sub-thread.  This means that the animation will not
 * necessarily be available at the time this method returns.  You may still use
 * the returned AnimControl immediately, though, but no visible effect will
 * occur until the animation eventually becomes available.  You can test
 * AnimControl::is_pending() to see if the animation has been loaded yet, or
 * wait for it to finish with AnimControl::wait_pending() or even
 * PartBundle::wait_pending().  You can also set an event to be triggered when
 * the animation finishes loading with AnimControl::set_pending_done_event().
 */
PT(AnimControl) PartBundle::
 ...
 * Returns true if the joint is successfully frozen, or false if the named child
 * is not a joint (or slider) or does not exist.
 */
bool PartBundle::
...
/**
 * Specifies that the joint with the indicated name should be animated with the
 * transform on the indicated node.  It will henceforth always follow the node's
 * transform, regardless of any animations that may subsequently be bound to the
 * joint.  Returns true if the joint is successfully controlled, or false if the
 * named child is not a joint (or slider) or does not exist.
 */
bool PartBundle::
 ...
 * whatever transforms are dictated by the animation.  Returns true if the joint
 * is released, or false if the named child was not previously controlled or
...
/**
 * Updates all the parts in the bundle to reflect the data for the current frame
 * (as set in each of the AnimControls).  Returns true if any part has changed
 * as a result of this, or false otherwise.
 */
bool PartBundle::
update() {
 ...
 * uninitialized AnimControl and fills it in if the bind is successful.  Returns
 ...
 * PartBundle.  Normally called only by the PartBundleNode itself, for instance
 * when the bundle is flattened with another node.
 */
void PartBundle::
...
/**
 * Removes the PartBundleNode pointer from the set of nodes associated with the
 * PartBundle.  Normally called only by the PartBundleNode itself, for instance
 * when the bundle is flattened with another node.
 */
void PartBundle::
...
/**
 * Removes and stops all the currently activated AnimControls that animate some
 * joints also animated by the indicated AnimControl.  This is a special
...
/**
 * Takes in a vector of pointers to TypedWritable objects that correspond to all
