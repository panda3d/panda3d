 ...
 * because by the time we get here we're already partly destructed.  Instead, we
 * should call disconnect() from each specific kind of derived class.
 */
ClientDevice::
~ClientDevice() {
 ...
 * stop receiving updates.  Normally, you should not need to call this
 * explicitly (and it is probably a mistake to do so); it will automatically be
 * called when the ClientDevice object destructs.  The lock should *not* be held
 * while this call is made; it will explicitly grab the lock itself.
 */
void ClientDevice::
disconnect() {
  if (_is_connected) {
 ...
 * already polled everything this frame.  This should generally be called before
 * accessing the data in this ClientDevice to ensure that it is fresh.
 */
void ClientDevice::
poll() {
  _client->poll();
