 ...
 * true if the fork was successful, or false otherwise (for instance, because we
 * were already forked, or because asynchronous threads are disabled).
 */
bool ClientBase::
 ...
 * indicated device type.  The device_type should be one of ClientTrackerDevice,
 * ClientAnalogDevice, etc.; the device_name is implementation defined.
 * Normally, the user does not need to call this function directly; it is called
 * automatically by creating a TrackerNode or AnalogNode or some such data graph
 * node.  The return value is the pointer to the created device (which might be
 * the same pointer returned by a previous call to this function with the same
 * parameters).  When the pointer destructs (i.e.  its reference count reaches
 * zero) it will automatically be disconnected.  If the named device does not
 * exist or cannot be connected for some reason, NULL is returned.
 */
 ...
 * will no longer receive automatic updates with each poll.  The return value is
 * true if the device was disconnected, or false if it was unknown (e.g.  it was
 ...
 * fork_asynchronous_thread() was called; otherwise, it will be called once per
 * frame.
 */
void ClientBase::
do_poll() {
 ...
 * function must be static, so this merely calls the non-static member callback
 * In addition, the function has a void* return type even though we don't
 * actually return anything.  This is necessary because ipc assumes a function
 * that does not return anything indicates that the associated thread should  be
 * created as unjoinable (detached).
 */
void *ClientBase::
