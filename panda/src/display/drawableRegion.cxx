...
/**
 * Returns true if any of the clear types (so far there are just color or depth)
 * have been set active, or false if none of them are active and there is no
 * need to clear.
 */
bool DrawableRegion::
...
/**
 * Sets the amount by which the pixels of the region are scaled internally when
 * filling the image interally.  Setting this number larger makes the pixels
 * blockier, but may make the rendering faster, particularly for software
 * renderers.  Setting this number to 2.0 reduces the number of pixels that have
 * to be filled by the renderer by a factor of 2.0.  It doesn't make sense to
 * set this lower than 1.0.  It is possible to set this on either individual
 * DisplayRegions or on overall GraphicsWindows, but you will get better
 * performance for setting it on the window rather than its individual
 * DisplayRegions.  Also, you may not set it on a DisplayRegion that doesn't
 * have both clear_color() and clear_depth() enabled.  This property is only
 * supported on renderers for which it is particularly useful--currently, this
 * is the tinydisplay software renderer.  Other kinds of renderers allow you to
 * set this property, but ignore it.
 */
void DrawableRegion::
