 ...
 * properties will be applied to the window (if valid) at the next execution of
 * process_events().
 */
const WindowProperties GraphicsWindow::
 ...
 * request a window change size or minimize or something.  The change is not
 * made immediately; rather, the request is saved and will be applied the next
 * time the window task is run (probably at the next frame).
 */
void GraphicsWindow::
...
/**
 * Changes the name of the event that is generated when this window is modified
 * externally, e.g.  to be resized or closed by the user.  By default, all
 * windows have the same window event unless they are explicitly changed.  When
 * the event is generated, it includes one parameter: the window itself.
 */
void GraphicsWindow::
...
/**
 * Returns the name of the event that is generated when this window is modified
 * externally, e.g.  to be resized or closed by the user.  See
 * set_window_event().
 */
...
/**
 * Sets the event that is triggered when the user requests to close the window,
 * e.g.  via alt-F4, or clicking on the close box.  The default for each window
 * is for this event to be the empty string, which means the window-close
 * request is handled immediately by Panda (and the window will be closed
 * without the app getting a chance to intervene).  If you set this to a
 * nonempty string, then the window is not closed, but instead the event is
 * thrown.  It is then up to the app to respond appropriately, for instance by
 * presenting an "are you sure?"  dialog box, and eventually calling
 * close_window() when the user is sure.  It is considered poor form to set this
 * string and then not handle the event.  This can frustrate the user by making
 * it difficult for him to cleanly shut down the application (and may force the
 * user to hard-kill the app, or reboot the machine).
 */
void GraphicsWindow::
 ...
 * return value is the number of valid sizes that were found.  Note this doesn't
 * guarantee a resize attempt will work; you still need to check the return
 * value.  (It might be better to implement some sort of query interface that
 * returns an array of supported sizes, but this way is somewhat simpler and
 * will do the job on most cards, assuming they handle the std sizes the app
 * knows about.)
 */
int GraphicsWindow::
...
/**
 * Adds a GraphicsWindowInputDevice to the vector.  Returns the index of the new
 * device.
 */
int GraphicsWindow::
