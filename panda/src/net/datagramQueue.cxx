 ...
 * one of them up.  Returns true if successful, false if the queue was full.  If
 * block is true, this will not return until successful, waiting until the queue
 * has space available if necessary.
 */
bool DatagramQueue::
 ...
 * datagram is available, this will immediately return; otherwise, it will block
 * until a datagram becomes available.  Multiple threads may simultaneously
 * block on extract(); when a datagram is subsequently inserted into the queue,
 * one of the threads will return from extract() with the datagram.  The return
 * value is true if the datagram is successfully extracted, or false if the
 * queue was destroyed while waiting.  (In the case of a false return, the
 * thread should not attempt to operate on the queue again.)
 */
bool DatagramQueue::
...
/**
 * Sets the maximum size the queue is allowed to grow to.  This is primarily for
 * a sanity check; this is a limit beyond which we can assume something bad has
 * happened.  It's also a crude check against unfortunate seg faults due to the
 * queue filling up and quietly consuming all available memory.
 */
void DatagramQueue::
