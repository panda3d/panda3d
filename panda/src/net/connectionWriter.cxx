...
/**
 * Creates a new ConnectionWriter with the indicated number of threads to handle
 * output.  If num_threads is 0, all datagrams will be sent immediately instead
 * of queueing for later transmission by a thread.
 */
ConnectionWriter::
...
/**
 * Limits the number of packets that may be pending on the outbound queue.  This
 * only has an effect when using threads; if num_threads is 0, then all packets
...
/**
 * Enqueues a datagram for transmittal on the indicated socket.  Since the host
 * address is not specified with this form, this function should only be used
 * for sending TCP packets.  Use the other send() method for sending UDP
 * packets.  Returns true if successful, false if there was an error.  In the
 * normal, threaded case, this function only returns false if the send queue is
 * filled; it's impossible to detect a transmission error at this point.  If
 * block is true, this will not return false if the send queue is filled;
 ...
 * is appropriate for UDP packets.  Use the other send() method for sending TCP
 * packets.  Returns true if successful, false if there was an error.  In the
 * normal, threaded case, this function only returns false if the send queue is
 * filled; it's impossible to detect a transmission error at this point.  If
 * block is true, this will not return false if the send queue is filled;
 ...
 * datagram are simply sent down the pipe.  Setting the ConnectionWriter to raw
 * mode must be done with care.  This can only be done when the matching
 * ConnectionReader is also set to raw mode, or when the ConnectionWriter is
 * communicating to a process that does not expect datagrams.
 */
void ConnectionWriter::
 ...
 * a floating pointer.  This makes the ConnectionWriter invalid; presumably it
