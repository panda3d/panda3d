...
/**
 * If a previous call to new_connection_available() returned true, this function
 * will return information about the newly established connection.  The
 * rendezvous parameter is the particular rendezvous socket this new connection
 * originally communicated with; it is provided in case the ConnectionListener
 * was monitorind more than one and you care which one it was.  The address
 * parameter is the net address of the new client, and new_connection is the
 * socket of the newly established connection.  The return value is true if a
 * connection was successfully returned, or false if there was, in fact, no new
 * connection.  (This may happen if there are multiple threads accessing the
 * QueuedConnectionListener).
 */
bool QueuedConnectionListener::
...
/**
 * This flavor of get_new_connection() simply returns a new connection, assuming
 * the user doesn't care about the rendezvous socket that originated it or the
 ...
 * has been established.  The QueuedConnectionListener simply queues up this
 * fact for later retrieval by get_new_connection().
 */
