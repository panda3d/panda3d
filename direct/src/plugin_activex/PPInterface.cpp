...
HRESULT PPInterface::Invoke(int nType, IDispatch* pDisp, CString& ptName, VARIANT* pvResult, int cArgs, VARIANT* params)
{
    ...
    // Reversing the arguments!!! NOTE: http:msdn.microsoft.comen-
    // uslibrarycc237569(PROT.10).aspx pDispParams: MUST point to a DISPPARAMS
    // structure that defines the arguments passed to the method.  Arguments
    // MUST be stored in pDispParams->rgvarg in reverse order, so that the
    // first argument is the one with the highest index in the array.  Byref
    // arguments MUST be marked in this array as VT_EMPTY entries, and stored
    // in rgVarRef instead.
...
HRESULT PPInterface::P3DCallMethod( P3D_object* p3dObject, CString& name, DISPPARAMS FAR* pdispparams, VARIANT FAR* varResult )
{
    ...
    {
        // Reversing the arguments!!! http:msdn.microsoft.comen-
        // uslibrarycc237569(PROT.10).aspx pDispParams: MUST point to a
        // DISPPARAMS structure that defines the arguments passed to the
        // method.  Arguments MUST be stored in pDispParams->rgvarg in reverse
        // order, so that the first argument is the one with the highest index
        // in the array.  Byref arguments MUST be marked in this array as
        // VT_EMPTY entries, and stored in rgVarRef instead.
...
P3D_object* PPInterface::variant_to_p3dobj(COleVariant* variant)
{
    ...
    case VT_EMPTY:
        {
            // return P3D_new_none_object_ptr(); A.T. Panda really expect
            // undefined object here
            return P3D_new_undefined_object_ptr();
            break;
        }
    case VT_BOOL:
