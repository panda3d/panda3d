 ...
 * already recorded, if any.  Returns true if the file is successfully read,
 * false if there was an error (in which case the file might have been partially
 * read).
 */
bool DCFile::
...
/**
 * Returns a pointer to the one DCField that has the indicated index number, of
 * all the DCFields across all classes in the file.  This method is only valid
 * if dc-multiple-inheritance is set true in the Config.prc file.  Without this
 * setting, different DCFields may share the same index number, so this global
...
/**
 * Returns the number of symbols explicitly imported by the nth import line.  If
 * this is 0, the line is "import modulename"; if it is more than 0, the line is
 * "from modulename import symbol, symbol ... ".
 */
int DCFile::
...
/**
 * Returns the typedef that has the indicated name, or NULL if there is no such
 * typedef name.
 */
DCTypedef *DCFile::
...
/**
 * Returns the keyword that has the indicated name, or NULL if there is no such
 * keyword name.
 */
const DCKeyword *DCFile::
 ...
 * guaranteed to be consistent if the contents of the file have not changed, and
 * it is very likely to be different if the contents of the file do change.
 */
 ...
 * DCFile becomes the owner of the pointer and will delete it when it destructs.
 * Returns true if the class is successfully added, or false if there was a name
 * conflict.
 */
bool DCFile::
 ...
 * the owner of the pointer and will delete it when it destructs.  Returns true
 * if the switch is successfully added, or false if there was a name conflict.
 */
bool DCFile::
 ...
 * from the most-recently added module, e.g.  "from module_name import symbol".
 * If the list of symbols is empty, the syntax is taken to be "import
 * module_name".
 */
void DCFile::
 ...
 * DCFile becomes the owner of the pointer and will delete it when it destructs.
 * Returns true if the typedef is successfully added, or false if there was a
 * name conflict.
 */
bool DCFile::
 ...
 * DCFile.  These keywords may then be added to DCFields.  It is not an error to
 * add a particular keyword more than once.
 */
bool DCFile::
 ...
 * reported with the file, but will be deleted when the DCFile object destructs.
...
/**
 * Sets the next sequential available index number on the indicated field.  This
 * is only meant to be called
 */
void DCFile::
