...
/**
 * Returns the type represented by this_type[size].  In the case of a
 * DCArrayParameter, this means it modifies the current type to append the array
 * specification on the innermost type, and returns this same pointer again.
 */
DCParameter *DCArrayParameter::
 ...
 * read from the get_num_length_bytes() stored in the stream on the pack).  This
 * will only be called if get_num_length_bytes() returns nonzero.
 */
int DCArrayParameter::
...
/**
 * Packs the arrayParameter's specified default value (or a sensible default if
 * no value is specified) into the stream.  Returns true if the default value is
 * packed, false if the arrayParameter doesn't know how to pack its default
 * value.
 */
bool DCArrayParameter::
