 ...
 * defined for this token.  This is intended to implement test versions and the
 * like, for instance in which a particular p3d file may reference a package on
 * one particular host, but there is an alternate version to be tested on a
 * different host.  The HTML code that embeds the p3d file can choose to set the
 * alt_host token to redirect the p3d file to the alternate host.  The actual
 * URL for the alternate host is embedded within the host's contents.xml as a
 * security measure, to prevent people from tricking a p3d file into running
 * untrusted code by redirecting it to an arbitrary URL.
 */
P3DHost *P3DHost::
...
/**
 * Returns true if a contents.xml file has been successfully read for this host
 * and is still current, false otherwise.
 */
bool P3DHost::
...
/**
 * Reads the contents.xml file in the standard filename, if possible.  Returns
 ...
 * the contents.xml file into the standard location (if it's not there already).
 ...
 * file, or from a p3d_info.xml file.
 */
void P3DHost::
...
/**
 * Returns a (possibly shared) pointer to the indicated package.  The
 * package_seq value should be the expected minimum package_seq value for the
...
/**
 * Chooses the most appropriate platform for the indicated package based on what
 * this hardware supports and what is actually available.  Also fills in
...
/**
 * Fills the indicated FileSpec with the hash information for the package's desc
 * file, and also determines the package's platform.  Returns true if
...
/**
 * Removes the indicated package from the cache of packages known by this host.
 * This is invoked from the Python side by AppRunner.deletePackages(), so that
 * we remove the package before deleting its files.
 */
void P3DHost::
 ...
 * alt_host, after downloading the contents.xml file for this file and learning
 * the true URL for its target alt_host.
 */
void P3DHost::
 ...
 * the root of the host's install tree.  Stores the result in _host_dir.  This
 * code is duplicated in Python, in HostInfo.determineHostDir().
 */
void P3DHost::
 ...
 * filename to lowercase.  On Windows, it further replaces forward slashes with
 * backslashes.
 */
string P3DHost::
