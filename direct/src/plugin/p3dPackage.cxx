  ...
  _alt_host(alt_host)
{
  ...
  // This is set true if the package is a "solo", i.e.  a single file, instead
  // of an xml file and a multifile to unpack.
  _package_solo = false;
...
activate_download() {
  ...
  if (_ready) {
    ...
    // Otherwise, if we've already got the desc file, then start the download.
    if (_info_ready) {
...
add_instance(P3DInstance *inst) {
  ...
  if (!_host->has_current_contents_file(inst_mgr)) {
    // If the host needs to update its contents file, we're no longer sure
    // that we're current.
    _info_ready = false;
    _ready = false;
    _failed = false;
...
/**
 * Indicates that the given instance is no longer interested in this package
 * and will not be responsible for downloading it.
 */
void P3DPackage::
...
mark_used() {
  ...
  // Unlike the Python variant of this function, we don't mess around with
  // updating the disk space or anything.
  ...
  if (_updated) {
    // If we've updated the package, we're no longer sure what its disk space
    // is.  Remove that from the XML file, so that the Python code can
    // recompute it later.
    xusage->RemoveAttribute("disk_space");
    xusage->SetAttribute("last_update", (int)now);
  }

  // Write the file to a temporary filename, then atomically move it to its
  // actual filename, to avoid race conditions.
  ostringstream strm;
...
/**
 * Removes the package directory and all its contents from the user's hard
 * disk.
 */
void P3DPackage::
uninstall() {
  ...
  // First, make sure that all instances that are sharing this package are
  // stopped, so there will be no access conflicts preventing us from removing
  // the files.  This is particularly important on Windows.
  Instances::iterator ii;
  for (ii = _instances.begin(); ii != _instances.end(); ++ii) {
    P3DInstance *inst = (*ii);
...
/**
 * Returns a newly-allocated XML structure that corresponds to the package
 * data within this instance.
 */
...
download_contents_file() {
  ...
  if (!_host->has_contents_file() && inst_mgr->get_verify_contents() != P3D_VC_force) {
    // First, read whatever contents file is already on disk.  Maybe it's
    // current enough.
    _host->read_contents_file();
  }

  if (_host->has_current_contents_file(inst_mgr)) {
    // We've already got a contents.xml file; go straight to the package desc
    // file.
 ...
 * local copy is stale.  This is called only from download_desc_file(), or
 * from Download::download_finished().  If the former, the download pointer
 * will be NULL.
 *
 * If it turns out a new version can be downloaded, the indicated Download
 * object (and the current install plan) is discarded, and the package
 * download is restarted from the beginning.
 *
 * If there is no new version available, calls resume_download_finished() on
 * the indicated Download object, to carry on as if nothing had happened.
 */
void P3DPackage::
redownload_contents_file(P3DPackage::Download *download) {
  ...
  if (_host->get_contents_iseq() != _host_contents_iseq) {
    ...
    // anything--just go restart the download.
    host_got_contents_file();
    return;
  }
...
contents_file_redownload_finished(bool success) {
  ...
  if (_host->get_contents_iseq() != _host_contents_iseq) {
    // If the contents_iseq number has changed, we don't even need to bother
    // reading what we just downloaded.
    contents_changed = true;
  }

  if (!contents_changed && success) {
    // If we successfully downloaded something, see if it's different from
    // what we had before.
  ...
  if (contents_changed) {
    ...
    // Nothing's changed.  This was just a useless diversion.  We now return
...
host_got_contents_file() {
  ...
  if (!_alt_host.empty()) {
    // If we have an alt host specification, maybe we need to change the host
    // now.
    ...
    // Clear the alt_host string now that we're migrated to our final host.
    _alt_host.clear();
  ...
  // Record this now, so we'll know later whether the host has been reloaded
  // (e.g.  due to some other package, from some other instance, reloading
  // it).
  _host_contents_iseq = _host->get_contents_iseq();

  // Now adjust the platform based on the available platforms provided.
...
/**
 * Starts downloading the desc file for the package, if it's needed; or read
 * the local version if it's fresh enough.
 */
void P3DPackage::
download_desc_file() {
  assert(!_package_dir.empty());

  // Attempt to check the desc file for freshness.  If it already exists, and
  // is consistent with the server contents file, we don't need to re-download
  // it.
  string package_seq;
  ...
  // The desc file might have a different path on the host server than it has
  // locally, because we might strip out the platform directory locally
  // (according to _per_platform).
...
got_desc_file(TiXmlDocument *doc, bool freshly_downloaded) {
  ...
  if (per_platform != _per_platform) {
    ...
    // _per_platform we got from the contents.xml file has to apply, because
    // we're already committed to the _package_dir we're using.
  }
  ...
  if (inst_mgr->get_verify_contents() == P3D_VC_never) {
    // This means we'll just leave it at this and assume that we're finished.
    report_done(true);
    return;
  }

  // Get a list of all of the files in the directory, so we can remove files
  // that don't belong.
  ...
  // Now, any files that are still in the contents list don't belong.  It's
  // important to remove these files before we start verifying the files that
  // we expect to find here, in case there is a problem with ambiguous
  // filenames or something (e.g.  case insensitivity).
  ...
  if (all_extracts_ok) {
    ...
    // We need to get the file data still, but at least we know all about it
    // by this point.
    build_install_plans(doc);

    if (!_allow_data_download) {
      // Not authorized to start downloading yet; just report that we're
      // ready.
      report_info_ready();
    } else {
...
/**
 * Sets up _install_plans, a list of one or more "plans" to download and
 * install the package.
 */
void P3DPackage::
build_install_plans(TiXmlDocument *doc) {
  ...
  if (!_uncompressed_archive.quick_verify(_package_dir)) {
    ...
    if (!_compressed_archive.quick_verify(_package_dir)) {
      // The compressed archive is no good either.  Download a new compressed
      // archive.
    ...
    // Uncompress the compressed archive to generate the uncompressed archive.
  ...
  if (needs_redownload) {
    // Since we need to do some downloading, try to build a plan that involves
    // downloading patches instead of downloading the whole file.  This will
    // be our first choice, plan A, if we can do it.

    // We'll need the md5 hash of the uncompressed archive currently on disk.
...
/**
 * Performs the next step in the current install plan.
 *
 * If download_finished is false, there is a pending download that has not
 * fully completed yet; otherwise, download_finished should be set true.
 *
 * If plan_failed is false, it means that the top-of-stack plan is still good;
 * if true, the top-of-stack plan has failed and should be removed.
 */
void P3DPackage::
...
report_done(bool success) {
  ...
  if (!_allow_data_download && success) {
    // If we haven't been authorized to start downloading yet, just report
...
/**
 * Initiates a download of the indicated file.  Returns the new Download
 * object.
 */
...
start_download(P3DPackage::DownloadType dtype, const string &urlbase,
  ...
  // We can't explicitly support partial downloads here, because Mozilla
  // provides no interface to ask for one.  We have to trust that Mozilla's
  // use of the browser cache handles partial downloads for us automatically.
  ...
  // Fill up the _try_urls vector for URL's to try getting this file from, in
  // reverse order.
  bool is_contents_file = (dtype == DT_contents_file || dtype == DT_redownload_contents_file);

  // The last thing we try is the actual authoritative host, with a cache-
  ...
  if (dtype == DT_redownload_contents_file) {
    // When we're redownloading the contents file after a download error, we
    // always go straight to the authoritative host, not even to the super-
    // mirror.

  } else {
    // In other cases, if the "super mirror" is enabled, we try that first.
  ...
  if (download->_try_urls.size() == 1) {
    // If we only ended up with only one URL on the try list, then try it
    // twice, for a bit of redundancy in case there's a random network hiccup
    // or something.

    // Save a copy into its own string object first to avoid self-
    // dereferencing errors in the push_back() method.
 ...
 * indicates that the instance owning this download object is terminating and
 * we should either find a new instance or abort the download.
 *
 * The return value is true if a new instance is available, or false if not.
 */
bool P3DPackage::
instance_terminating(P3DInstance *instance) {
  ...
  // There are more instances available to continue this download; pick one of
  // them.  Move this one to the end of the list.
...
download_finished(bool success) {
  ...
  if (success && !_file_spec.get_filename().empty()) {
    // We think we downloaded it correctly.  Check the hash to be sure.
  ...
  if (!success) {
    ...
    // Maybe it failed because our contents.xml file is out-of-date.  Go try
    // to freshen it.
...
do_step(bool download_finished) {
  ...
  if (_download->get_download_success()) {
    ...
    // The download was interrupted because its instance is shutting down.
    // Don't try any other plans, unless we have some more instances.
    P3DInstance *instance = _download->get_instance();
    if (!_package->instance_terminating(instance)) {
      // That was the only instance referencing this package, so stop the
      // download.
    ...
    // The Download object has already tried all of the mirrors, and they all
    // failed.
...
do_step(bool download_finished) {
  // This method is called within the main thread.  It simply checks the
  // thread status, and returns.
...
thread_main() {
  // This method is called within the sub-thread.  It calls thread_step() to
  // do its work.
...
/**
 * Should be called from time to time within the sub-thread to update the
