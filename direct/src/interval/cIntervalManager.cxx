 ...
 * interval.  This index will be unique among all the currently added intervals,
 * but not unique across all intervals ever added to the manager.  The maximum
 * index value will never exceed the maximum number of intervals added at any
 * given time.  If the external flag is true, the interval is understood to also
 * be stored in the scripting language data structures.  In this case, it will
 * be available for information returned by get_next_event() and
 * get_next_removal().  If external is false, the interval's index will never be
 * returned by these two functions.
 */
...
/**
 * Pauses or finishes (removes from the active queue) all intervals tagged with
 * auto_pause or auto_finish set to true.  These are intervals that someone
 * fired up but won't necessarily expect to clean up; they can be interrupted at
 * will when necessary.  Returns the number of intervals affected.
 */
int CIntervalManager::
interrupt() {
  MutexHolder holder(_lock);
 ...
 * and that has not yet been removed.  After each call to step(), the scripting
 * language should call get_next_event() and get_next_removal() repeatedly to
 * process all the high-level (e.g.  Python-interval-based) events and to manage
 * the high-level list of intervals.
 */
void CIntervalManager::
step() {
  MutexHolder holder(_lock);
 ...
 * events pending.  If this function returns something other than -1, it is the
 * scripting language's responsibility to query the indicated interval for its
 * next event via get_event_index(), and eventually pop_event().  Then
 * get_next_event() should be called again until it returns -1.
 */
int CIntervalManager::
get_next_event() {
  MutexHolder holder(_lock);
 ...
 * if no intervals were removed.  If this returns something other than -1, the
 * scripting language should clean up its own data structures accordingly, and
...
/**
 * Removes the indicated index number from the active list, either by moving it
 * to the removed queue if it is flagged external, or by simply making the slot
 * available again if it is not.  Assumes the lock is already held.
 */
void CIntervalManager::
