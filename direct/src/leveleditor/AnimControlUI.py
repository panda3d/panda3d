...
class TimeSlider(wx.Window):
    ...
    def DrawTimeSlider(self, dc):
        ...
        h = int(self.h)
        unit = self.w / float(self.frameNum)
        ...
        step = None
        for maxFrame, s in rules:
            if self.frameNum <= maxFrame:
                step = s
                break
        ...
        start_index = step - 1
        x = int(unit * start_index)

        count = int(self.frameNum / step)
        ...
        for i in range(count):
            ...
            self.numbers.append(self.sliderStartFrame + i * step)
            x += int(unit * step)
    ...
    def DrawFrame(self, dc):
        if not hasattr(self, "unitWidth"):
            if self.frameNum > 0:
                self.unitWidth = float(self.w) / float(self.frameNum)
            else:
                self.unitWidth = 1.0
        ...
        frameWidth = int(self.unitWidth)
        frameHeight = self.h
        frameX = int(self.unitWidth * i)
        frameY = 0

        self.curRect = wx.Rect(frameX, frameY, frameWidth, frameHeight)
...
class TimeRange(wx.Window):
    ...
    def DrawTimeRange(self, dc):
        ...
        unitWidth = (self.w - 6.0) / float(self.frameNum)

        rectX = 3.0 + (self.sliderStartFrame - self.startFrame) * unitWidth
        rectY = 2.0
        rectWidth = (self.sliderEndFrame - self.sliderStartFrame + 1) * unitWidth
        rectHeight = self.h - 4.0

        curRect = wx.Rect(int(rectX), int(rectY), int(rectWidth), int(rectHeight))

        dc.DrawRoundedRectangle(curRect, radius=2)
