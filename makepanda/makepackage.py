...
def MakeInstallerOSX(version, python_versions=[], **kwargs):
    ...
    oscmd("mkdir -p dstroot/tools/Library/Developer/Tools")
    oscmd("ln -s ../Panda3D/bin dstroot/tools/Library/Developer/Tools/Panda3D")
    ...
    #    oscmd("mkdir -p dstroot/openal/Library/Developer/Panda3D/lib")
    #    oscmd("cp -R %s/lib/libp3openal_audio.* dstroot/openal/Library/Developer/Panda3D/lib/" % outputdir)
    ...
    if os.path.isdir("samples"):
        oscmd("mkdir -p dstroot/samples/Library/Developer/Examples/Panda3D")
        oscmd("cp -R samples/* dstroot/samples/Library/Developer/Examples/Panda3D/")
    ...
    for pkg in pkgs:
        ...
        elif os.path.exists("/Library/Developer/usr/bin/packagemaker"):
            cmd = '/Library/Developer/usr/bin/packagemaker --info /tmp/Info_plist --version ' + version + ' --out dstroot/Panda3D/Panda3D.mpkg/Contents/Packages/' + pkg + '.pkg ' + target + ' --domain system --root dstroot/' + pkg + '/ --no-relocate'
        ...
        elif os.path.exists("/Library/Developer/Tools/PackageMaker.app/Contents/MacOS/PackageMaker"):
            cmd = '/Library/Developer/Tools/PackageMaker.app/Contents/MacOS/PackageMaker --info /tmp/Info_plist --version ' + version + ' --out dstroot/Panda3D/Panda3D.mpkg/Contents/Packages/' + pkg + '.pkg ' + target + ' --domain system --root dstroot/' + pkg + '/ --no-relocate'
        elif os.path.exists("/Library/Developer/Tools/packagemaker"):
            cmd = '/Library/Developer/Tools/packagemaker -build -f dstroot/' + pkg + '/ -p dstroot/Panda3D/Panda3D.mpkg/Contents/Packages/' + pkg + '.pkg -i /tmp/Info_plist'
